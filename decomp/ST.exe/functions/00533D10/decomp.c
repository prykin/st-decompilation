#include "../../pseudocode_runtime.h"


/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */
/* Recovered from embedded debug metadata:
   E:\__titans\Andrey\optpanel.cpp
   OptPanelTy::GetMessage

   [STMessageHandlerApplier] Recovered common GetMessage envelope/signature.
   Evidence: family_entries=00533D10; family_names=OptPanelTy::GetMessage; ret4=78;
   direct_offsets={10:6,14:39,18:17,1c:6}

   [STSwitchEnumApplier] Switch target field_01A4 uses
   /SubmarineTitans/Recovered/Enums/OptPanelTy_field_01A4State. Cases:
   CASE_1=1;CASE_2=2;CASE_3=3;CASE_4=4;CASE_5=5;CASE_6=6;CASE_7=7;CASE_8=8;CASE_9=9;CASE_A=10;CASE_B=11;CASE_C=12;CASE_D=13;CASE_E=14

   [STSwitchEnumApplier] Switch target field_01AB uses
   /SubmarineTitans/Recovered/Enums/OptPanelTy_field_01ABState. Cases:
   CASE_1=1;CASE_2=2;CASE_3=3;CASE_4=4;CASE_5=5;CASE_6=6;CASE_7=7;CASE_A=10

   [STSwitchEnumApplier] Switch target field_01AB uses
   /SubmarineTitans/Recovered/Enums/OptPanelTy_field_01ABState. Cases:
   CASE_1=1;CASE_2=2;CASE_3=3;CASE_4=4;CASE_5=5;CASE_6=6;CASE_7=7;CASE_A=10;CASE_B=11 */

int __thiscall OptPanelTy::GetMessage(OptPanelTy *this,STMessage *message)

{
  undefined1 *puVar1;
  char cVar2;
  OptPanelTy_field_01A4State OVar3;
  ushort uVar4;
  STMessageId SVar5;
  DArrayTy *pDVar6;
  InternalExceptionFrame *pIVar7;
  undefined1 uVar8;
  int iVar9;
  DWORD DVar10;
  uint uVar11;
  undefined4 *puVar12;
  LPSTR pCVar13;
  cMf32 *pcVar14;
  byte *pbVar15;
  uint uVar16;
  void *pvVar17;
  OptPanelTy *this_00;
  ushort *puVar18;
  int *piVar19;
  char *pcVar20;
  undefined4 *puVar21;
  char *pcVar22;
  char *pcVar23;
  byte *pbVar24;
  bool bVar25;
  uint uVar26;
  uint *puVar27;
  byte bVar28;
  int iVar29;
  code *pcVar30;
  ccFntTy *pcVar31;
  int iVar32;
  undefined4 uVar33;
  UINT UVar34;
  code *pcVar35;
  ulong uVar36;
  InternalExceptionFrame local_268;
  InternalExceptionFrame local_224;
  InternalExceptionFrame local_1e0;
  InternalExceptionFrame local_19c;
  InternalExceptionFrame local_158;
  InternalExceptionFrame local_114;
  InternalExceptionFrame local_d0;
  InternalExceptionFrame local_8c;
  undefined4 local_48;
  uint local_44;
  undefined4 local_3c;
  undefined4 local_38;
  undefined4 local_34;
  int local_30;
  int local_2c;
  int local_28;
  uint local_24;
  ushort *local_20;
  uint *local_1c;
  dword local_18;
  undefined4 *local_14;
  OptPanelTy *local_10;
  cMf32 *local_c;
  uint local_8;

  local_8c.previous = g_currentExceptionFrame;
  g_currentExceptionFrame = &local_8c;
  local_10 = this;
  iVar9 = Library::MSVCRT::__setjmp3(local_8c.jumpBuffer,0);
  this_00 = local_10;
  if (iVar9 != 0) {
    g_currentExceptionFrame = local_8c.previous;
    iVar29 = ReportDebugMessage("E:\\__titans\\Andrey\\optpanel.cpp",0x7f4,0,iVar9,
                                "%s","OptPanelTy::GetMessage");
    if (iVar29 != 0) {
      STDebugBreak(); /* noreturn in standalone pseudocode */
    }
    RaiseInternalException(iVar9,0,"E:\\__titans\\Andrey\\optpanel.cpp",0x7f4);
    return 0xffff;
  }
  if (message->id == MESS_ID_CREATE) {
    PreInitOptPanel(local_10);
  }
  PanelTy::GetMessage((PanelTy *)this_00,message);
  switch(message->id) {
  case MESS_OPTPANELTY_C001:
  case MESS_OPTPANELTY_C003:
  case MESS_SHARED_C09F:
  case MESS_BEHPANELTY_C0A0:
  case MESS_FSGSTY_C0A1:
  case MESS_SHARED_C0A2:
  case MESS_SHARED_C0A3:
  case MESS_BLDLABPANELTY_C0A4:
  case MESS_FRMPANELTY_C0A5:
  case MESS_BLDLABPANELTY_C0A4|MESS_ID_CREATE:
  case MESS_BLDLABPANELTY_C0A4|MESS_SHARED_0003:
  case MESS_BEHPANELTY_C0A0|MESS_SHARED_0008:
  case MESS_BEHPANELTY_C0A0|MESS_CURSORCLASSTY_0009:
  case MESS_BEHPANELTY_C0A0|MESS_SYSTEMCLASSTY_000A:
  case 0xc0ab:
  case 0xc0ac:
  case MESS_BEHPANELTY_C0A0|MESS_SHARED_0008|MESS_SHARED_0005:
  case MESS_OPTPANELTY_C0AE:
    if (((message->arg2).u32 != 0) &&
       ((uVar11 = *(uint *)((message->arg2).u32 + 4), uVar11 == 2 || ((3 < uVar11 && (uVar11 < 6))))
       )) {
      thunk_FUN_005252c0(0xae);
    }
  }
  SVar5 = message->id;
  if (SVar5 < 0xc007) {
    if (SVar5 == MESS_OPTPANELTY_C006) {
      pcVar35 = thunk_FUN_00529fe0;
      pCVar13 = thunk_FUN_00571240("BUT_MINUS",0);
      bVar28 = 1;
    }
    else {
      if (SVar5 < MESS_OPTPANELTY_8165) {
        if (SVar5 != MESS_OPTPANELTY_8164) {
          if (MESS_SHARED_8160 < SVar5) {
            if (SVar5 == MESS_SIDTY_8161) {
              puVar18 = this_00->field_01E5;
              uVar11 = *(uint *)(puVar18 + 10);
              if (uVar11 == 0) {
                uVar11 = ((uint)puVar18[7] * *(int *)(puVar18 + 2) + 0x1f >> 3 & 0x1ffffffc) *
                         *(int *)(puVar18 + 4);
              }
              local_8 = (DAT_0080874e != '\x03') - 1 & 0x73;
              puVar12 = (undefined4 *)FUN_006b4fa0((int)puVar18);
              uVar8 = (undefined1)local_8;
              for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
                *puVar12 = CONCAT22(CONCAT11(uVar8,uVar8),CONCAT11(uVar8,uVar8));
                puVar12 = puVar12 + 1;
              }
              for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
                *(undefined1 *)puVar12 = uVar8;
                puVar12 = (undefined4 *)((int)puVar12 + 1);
              }
              ccFntTy::SetSurf(this_00->field_017C,(int)this_00->field_01E5,0,0,0,0,0);
              if (this_00->field_01A4 == CASE_E) {
                pDVar6 = this_00->field_02FD;
                uVar11 = (uint)(message->arg0).words.low;
                if (uVar11 < pDVar6->count) {
                  /* ST_PSEUDO[dynamic_array_indexing]: expected DArrayAt<T>(pDVar6, uVar11) (runtime stride) */
                  puVar27 = (uint *)(pDVar6->elementSize * uVar11 + (int)pDVar6->data);
                }
                else {
                  puVar27 = (uint *)0x0;
                }
                uVar11 = (uint)((message->arg0).words.high == 0);
              }
              else {
                uVar11 = (uint)(message->arg0).words.low;
                if ((int)uVar11 < (int)this_00->field_01E9->elementSize) {
                  puVar27 = *(uint **)(this_00->field_01E9->growCapacity + uVar11 * 4);
                  uVar11 = (uint)((message->arg0).words.high == 0);
                }
                else {
                  puVar27 = (uint *)0x0;
                  uVar11 = (uint)((message->arg0).words.high == 0);
                }
              }
              ccFntTy::WrStr(this_00->field_017C,puVar27,2,-1,uVar11);
              Library::DKW::WGR::FUN_006b55f0
                        ((AnonShape_006B5B10_E0D06CF1 *)this_00->field_0068,0,
                         (message->arg1).words.low + 0x26,(message->arg1).words.high + 0x31,
                         (byte *)this_00->field_01E5,0,0,0,200,*(int *)(this_00->field_01E5 + 4));
              Library::DKW::DDX::FUN_006b3640
                        ((int *)PTR_008075a8,this_00->field_0060,0xffffffff,this_00->field_003C,
                         this_00->field_0044);
              g_currentExceptionFrame = local_8c.previous;
              return 0;
            }
            if (SVar5 != MESS_OPTPANELTY_8162) {
              if (SVar5 != MESS_SIDTY_8163) {
                g_currentExceptionFrame = local_8c.previous;
                return 0;
              }
              OVar3 = this_00->field_01A4;
              if (OVar3 == CASE_3) {
                if ((message->arg0).i32 < (int)this_00->field_01E9->elementSize) {
                  pcVar20 = *(char **)(this_00->field_01E9->growCapacity + (message->arg0).i32 * 4);
                }
                else {
                  pcVar20 = (char *)0x0;
                }
                Library::DKW::TBL::FUN_006b6020(&this_00->field_01F1->flags,0,pcVar20);
                this_00->field_0028 = 0x33;
                *(DArrayTy **)&this_00->field_0x2c = this_00->field_01F1;
                FUN_006e6080(this_00,2,this_00->field_01B9,(undefined4 *)&this_00->field_0x18);
                g_currentExceptionFrame = local_8c.previous;
                return 0;
              }
              if (OVar3 == CASE_4) {
                puVar18 = this_00->field_01E5;
                uVar11 = *(uint *)(puVar18 + 10);
                if (uVar11 == 0) {
                  uVar11 = ((uint)puVar18[7] * *(int *)(puVar18 + 2) + 0x1f >> 3 & 0x1ffffffc) *
                           *(int *)(puVar18 + 4);
                }
                local_8 = (DAT_0080874e != '\x03') - 1 & 0x73;
                puVar12 = (undefined4 *)FUN_006b4fa0((int)puVar18);
                uVar8 = (undefined1)local_8;
                for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
                  *puVar12 = CONCAT22(CONCAT11(uVar8,uVar8),CONCAT11(uVar8,uVar8));
                  puVar12 = puVar12 + 1;
                }
                for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
                  *(undefined1 *)puVar12 = uVar8;
                  puVar12 = (undefined4 *)((int)puVar12 + 1);
                }
                this_00->field_0028 = 0x26;
                FUN_006e6080(this_00,2,this_00->field_01B5,(undefined4 *)&this_00->field_0x18);
                ccFntTy::SetSurf(this_00->field_017C,(int)this_00->field_01E5,0,0,0,0,0);
                if (*(int *)&this_00->field_0x2c < (int)this_00->field_01E9->elementSize) {
                  puVar27 = *(uint **)(this_00->field_01E9->growCapacity +
                                      *(int *)&this_00->field_0x2c * 4);
                }
                else {
                  puVar27 = (uint *)0x0;
                }
                ccFntTy::WrStr(this_00->field_017C,puVar27,0,-1,0);
                puVar18 = this_00->field_01E5;
                Library::DKW::WGR::FUN_006b55f0
                          ((AnonShape_006B5B10_E0D06CF1 *)this_00->field_0068,0,0x26,0x1c,
                           (byte *)puVar18,0,0,0,*(int *)(puVar18 + 2),*(int *)(puVar18 + 4));
                g_currentExceptionFrame = local_8c.previous;
                return 0;
              }
              if (OVar3 != CASE_E) {
                g_currentExceptionFrame = local_8c.previous;
                return 0;
              }
              puVar18 = this_00->field_01E5;
              uVar11 = *(uint *)(puVar18 + 10);
              if (uVar11 == 0) {
                uVar11 = ((uint)puVar18[7] * *(int *)(puVar18 + 2) + 0x1f >> 3 & 0x1ffffffc) *
                         *(int *)(puVar18 + 4);
              }
              local_8 = (DAT_0080874e != '\x03') - 1 & 0x73;
              puVar12 = (undefined4 *)FUN_006b4fa0((int)puVar18);
              uVar8 = (undefined1)local_8;
              for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
                *puVar12 = CONCAT22(CONCAT11(uVar8,uVar8),CONCAT11(uVar8,uVar8));
                puVar12 = puVar12 + 1;
              }
              for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
                *(undefined1 *)puVar12 = uVar8;
                puVar12 = (undefined4 *)((int)puVar12 + 1);
              }
              ccFntTy::SetSurf(this_00->field_017C,(int)this_00->field_01E5,0,0,0,0,0);
              iVar9 = thunk_FUN_00648a30((uint)DAT_0080874d,(char *)&DAT_0080f33a);
              uVar33 = 0;
              iVar32 = -1;
              iVar29 = 0;
              if (iVar9 == 0) {
                puVar27 = (uint *)LoadResourceString(0x2711,g_module_00807618);
              }
              else {
                puVar27 = &DAT_0080f33a;
              }
              ccFntTy::WrStr(this_00->field_017C,puVar27,iVar29,iVar32,uVar33);
              puVar18 = this_00->field_01E5;
              Library::DKW::WGR::FUN_006b55f0
                        ((AnonShape_006B5B10_E0D06CF1 *)this_00->field_0068,0,0x26,0x1c,
                         (byte *)puVar18,0,0,0,*(int *)(puVar18 + 2),*(int *)(puVar18 + 4));
              g_currentExceptionFrame = local_8c.previous;
              return 0;
            }
            OVar3 = this_00->field_01A4;
            if (OVar3 < CASE_3) {
              g_currentExceptionFrame = local_8c.previous;
              return 0;
            }
            if ((CASE_4 < OVar3) && (OVar3 != CASE_E)) {
              g_currentExceptionFrame = local_8c.previous;
              return 0;
            }
            goto switchD_00535190_caseD_3;
          }
          if (SVar5 == MESS_SHARED_8160) {
            message->arg1 = *(STMessageArg *)(this_00->field_01E5 + 4);
            g_currentExceptionFrame = local_8c.previous;
            return 0;
          }
          if (MESS_SHARED_0003 < SVar5) {
            if (SVar5 != MESS_SHARED_6332) {
              g_currentExceptionFrame = local_8c.previous;
              return 0;
            }
            local_18 = (message->arg2).u32;
            pcVar31 = this_00->field_017C;
            if (pcVar31->field_00A0 != 0) {
              FUN_00710790((AnonShape_00710790_4CBB90D4 *)pcVar31);
            }
            local_8 = *(uint *)&pcVar31->field_0x8a;
            FUN_006b4170((AnonShape_006B5B10_E0D06CF1 *)this_00->field_0068,0,0x26,0x1a,0xca,0x6f,
                         (DAT_0080874e != '\x03') - 1U & 0x73);
            uVar16 = (uint)(message->arg0).words.high;
            uVar11 = uVar16;
            /* ST_PSEUDO[raw_pointer_offset]: candidate structure field after proof; otherwise retain buffer arithmetic */
            if ((int)uVar16 < (int)(*(int *)(local_18 + 0x1e0) + uVar16)) {
              do {
                if ((int)uVar11 < (int)this_00->field_02F9[2]) {
                  local_1c = *(uint **)(this_00->field_02F9[5] + uVar11 * 4);
                }
                else {
                  local_1c = (uint *)0x0;
                }
                if (local_1c != (uint *)0x0) {
                  ccFntTy::SetSurf(this_00->field_017C,this_00->field_0068,0,0x26,
                                   (uVar11 - uVar16) * local_8 + 0x1b,0xca,local_8);
                  ccFntTy::WrStr(this_00->field_017C,local_1c,0,-1,0);
                }
                uVar16 = (uint)(message->arg0).words.high;
                uVar11 = uVar11 + 1;
              /* ST_PSEUDO[raw_pointer_offset]: candidate structure field after proof; otherwise retain buffer arithmetic */
              } while ((int)uVar11 < (int)(*(int *)(local_18 + 0x1e0) + uVar16));
            }
            Library::DKW::DDX::FUN_006b3640
                      ((int *)PTR_008075a8,this_00->field_0060,0xffffffff,this_00->field_003C,
                       this_00->field_0044);
            g_currentExceptionFrame = local_8c.previous;
            return 0;
          }
          if (SVar5 == MESS_SHARED_0003) {
            DoneOptPanel(this_00);
            g_currentExceptionFrame = local_8c.previous;
            return 0;
          }
          if (SVar5 != MESS_ID_NONE) {
            if (SVar5 != MESS_ID_CREATE) {
              g_currentExceptionFrame = local_8c.previous;
              return 0;
            }
            InitOptPanel(this_00);
            g_currentExceptionFrame = local_8c.previous;
            return 0;
          }
          OVar3 = this_00->field_01A4;
          if ((CASE_2 < OVar3) &&
             ((((OVar3 < CASE_5 || (OVar3 == CASE_7)) && (this_00->field_01DD != (HANDLE)0x0)) &&
              (DVar10 = WaitForSingleObject(this_00->field_01DD,0), DVar10 == 0)))) {
            FindNextChangeNotification(this_00->field_01DD);
            if (this_00->field_01A4 != CASE_7) {
              PrepFiles(this_00,this_00->field_01E1);
            }
            this_00->field_0028 = 5;
            piVar19 = &this_00->field_01B5;
            iVar9 = 10;
            do {
              if (*piVar19 != 0) {
                FUN_006e6080(this_00,2,*piVar19,(undefined4 *)&this_00->field_0x18);
              }
              piVar19 = piVar19 + 1;
              iVar9 = iVar9 + -1;
            } while (iVar9 != 0);
          }
          if (this_00->field_0172 == CASE_3) {
            iVar9 = this_00->field_0050;
            if (this_00->field_0044 < iVar9) {
              this_00->field_0044 = this_00->field_0044 + 0x1e;
            }
            if (iVar9 <= this_00->field_0044) {
              this_00->field_0044 = iVar9;
              this_00->field_0172 = CASE_1;
              ShiftControls(this_00,1);
            }
            uVar11 = this_00->field_0044;
            uVar16 = this_00->field_003C;
            uVar26 = this_00->field_0060;
          }
          else {
            if (this_00->field_0172 != CASE_4) goto LAB_00534016;
            iVar9 = this_00->field_0050 - this_00->field_0048;
            if (iVar9 < this_00->field_0044) {
              this_00->field_0044 = this_00->field_0044 + -0x1e;
            }
            if (this_00->field_0044 <= iVar9) {
              DAT_0080878c = 0;
              local_38 = 0;
              local_34 = 0;
              local_3c = 1;
              if (this_00->field_019C != 0x6102) {
                thunk_FUN_0054edf0((undefined4 *)0x5,&local_3c,1,0xffffffff);
              }
              this_00->field_0044 = this_00->field_0050 - this_00->field_0048;
              this_00->field_0172 = CASE_2;
              if (this_00->field_01A0 != 0) {
                this_00->field_01A4 = CASE_1;
                SetOptControls(this_00);
                this_00->field_01A0 = 0;
              }
              if (this_00->field_019C != 0) {
                if ((this_00->field_01A4 == CASE_5) && (this_00->field_01AB == CASE_2)) {
                  _DAT_0080f32e = 1;
                  if (DAT_00808783 == '\x01') {
                    /* ST_PSEUDO[packed_or_unaligned_piece]: expected named packed member, bit extract/compose, or unaligned load */
                    _DAT_008087a0 = CONCAT31(_DAT_008087a1,9);
                  }
                  else if (DAT_00808783 == '\x02') {
                    /* ST_PSEUDO[packed_or_unaligned_piece]: expected named packed member, bit extract/compose, or unaligned load */
                    _DAT_008087a0 = CONCAT31(_DAT_008087a1,10);
                  }
                }
                memset(&this_00->field_0x18, 0, 0x20); /* compiler bulk-zero initialization */
                this_00->field_0024 = 1;
                this_00->field_0028 = this_00->field_019C;
                if (this_00->field_019C == 0x610a) {
                  *(undefined4 *)&this_00->field_0x2c = 6;
                }
                AppClassTy::PostNextMessage(this_00->field_0010,(undefined4 *)&this_00->field_0x18);
                this_00->field_019C = 0;
              }
            }
            uVar11 = this_00->field_0044;
            uVar16 = this_00->field_003C;
            uVar26 = this_00->field_0060;
          }
          Library::DKW::DDX::FUN_006b3640((int *)PTR_008075a8,uVar26,0xffffffff,uVar16,uVar11);
LAB_00534016:
          if ((uint)(this_00->field_0038 - this_00->field_0301) < 0x7d1) {
            g_currentExceptionFrame = local_8c.previous;
            return 0;
          }
          this_00->field_0301 = this_00->field_0038;
          uVar11 = thunk_FUN_00567060(0x807658);
          if (uVar11 != 0) {
            g_currentExceptionFrame = local_8c.previous;
            return 0;
          }
          thunk_FUN_00571320(&DAT_00807620,HWND_00856d78);
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        pcVar20 = "BUT_SLUP";
      }
      else {
        if (MESS_OPTPANELTY_C002 < SVar5) {
          if (SVar5 == MESS_OPTPANELTY_C003) {
            if ((this_00->field_01A4 == CASE_5) && (this_00->field_01AB == CASE_7)) {
              DAT_00808788 = 0;
            }
            if (this_00->field_01A9 == 0) {
              g_currentExceptionFrame = local_8c.previous;
              return 0;
            }
            this_00->field_01A4 = this_00->field_01A9;
            this_00->field_01A9 = this_00->field_01AA;
            this_00->field_01AA = 0;
            SetOptControls(this_00);
            g_currentExceptionFrame = local_8c.previous;
            return 0;
          }
          if (SVar5 != MESS_OPTPANELTY_C004) {
            if (SVar5 != MESS_SHARED_C005) {
              g_currentExceptionFrame = local_8c.previous;
              return 0;
            }
            pcVar35 = thunk_FUN_00529fe0;
            pCVar13 = thunk_FUN_00571240("BUT_PLUS",0);
            bVar28 = 1;
            goto LAB_00534654;
          }
          UVar34 = 0x274f;
          goto cf_common_exit_00536DB5;
        }
        if (SVar5 == MESS_OPTPANELTY_C002) {
          UVar34 = 0x274e;
          goto cf_common_exit_00536DB5;
        }
        if (MESS_OPTPANELTY_A106 < SVar5) {
          if (SVar5 != MESS_OPTPANELTY_C001) {
            g_currentExceptionFrame = local_8c.previous;
            return 0;
          }
          if ((this_00->field_01A4 == CASE_5) && (this_00->field_01AB == CASE_7)) {
            DAT_00808788 = 0;
          }
          this_00->field_01A0 = 1;
          message->id = MESS_CURSORCLASSTY_A101;
          /* ST_PSEUDO[raw_indirect_call]: expected typed vtable/callback call with explicit __thiscall receiver */
          (**(code **)g_cursorClass_00802A30->field_0000)(message);
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        if (0xa104 < SVar5) {
          if (this_00->field_01A4 != CASE_5) {
            g_currentExceptionFrame = local_8c.previous;
            return 0;
          }
          (message->arg2).u32 = 0;
          message->id = (SVar5 != MESS_OPTPANELTY_A105) + MESS_SHARED_C09F;
          /* ST_PSEUDO[raw_indirect_call]: expected typed vtable/callback call with explicit __thiscall receiver */
          (**(code **)this_00->field_0000)(message);
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        if (SVar5 != MESS_OPTPANELTY_8165) {
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        pcVar20 = "BUT_SLDN";
      }
      pcVar35 = thunk_FUN_00529fe0;
      pCVar13 = thunk_FUN_00571240(pcVar20,0);
      bVar28 = 6;
    }
LAB_00534654:
    UPanelTy::PaintBut((UPanelTy *)this_00,(AnonShape_0053D7A0_044B6141 *)message,bVar28,pCVar13,
                       pcVar35);
    g_currentExceptionFrame = local_8c.previous;
    return 0;
  }
  switch(SVar5) {
  case MESS_SHARED_C09F:
    OVar3 = this_00->field_01A4;
    switch(OVar3) {
    case CASE_1:
      this_00->field_01A9 = OVar3;
      this_00->field_01A4 = CASE_3;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_2:
      uVar8 = this_00->field_01A9;
      this_00->field_01A9 = OVar3;
      this_00->field_01AA = uVar8;
      this_00->field_01A4 = CASE_7;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_5:
      switch(this_00->field_01AB) {
      case CASE_1:
        goto switchD_00534835_caseD_1;
      case CASE_2:
        uVar11 = 0xffffffff;
        pcVar20 = &DAT_00807680;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = (char *)&DAT_0080ee1a;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        uVar11 = 0xffffffff;
        pcVar20 = PTR_s_SAVEGAME__0079acec;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = (char *)&DAT_0080ee1a;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        uVar11 = 0xffffffff;
        pcVar20 = (char *)&DAT_00807ddd;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = (char *)&DAT_0080ee1a;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        FUN_006b8280((char *)&DAT_0080ee1a,(char *)&DAT_0080ee1a);
        uVar11 = 0xffffffff;
        pcVar20 = PTR_DAT_0079ad00;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = (char *)&DAT_0080ee1a;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        uVar11 = 0xffffffff;
        pcVar20 = &this_00->field_0x1f5;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = (char *)&DAT_0080ee1a;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        SwitchOptPanel(this_00,-1);
        this_00->field_019C = 0x6100;
        DAT_0080879c = 1;
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      case CASE_3:
        wsprintfA((LPSTR)&DAT_0080ee1a,"%s%s",&DAT_00807680,&DAT_0080ef1e);
        SwitchOptPanel(this_00,-1);
        uVar11 = 0xffffffff;
        this_00->field_019C = 0x6100;
        pcVar20 = (char *)&DAT_0080ee1a;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = &DAT_0080ed16;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        STAppC::sub_0056E9E0((STAppC *)&DAT_00807620,0);
        puVar12 = &DAT_0080a14f;
        puVar21 = &DAT_008087b6;
        for (iVar9 = 0x666; iVar9 != 0; iVar9 = iVar9 + -1) {
          *puVar21 = *puVar12;
          puVar12 = puVar12 + 1;
          puVar21 = puVar21 + 1;
        }
        *(undefined1 *)puVar21 = *(undefined1 *)puVar12;
        puVar12 = (undefined4 *)&DAT_0080baec;
        puVar21 = &DAT_0080e43b;
        for (iVar9 = 0x100; iVar9 != 0; iVar9 = iVar9 + -1) {
          *puVar21 = *puVar12;
          puVar12 = puVar12 + 1;
          puVar21 = puVar21 + 1;
        }
        if (DAT_0080ed12 != (undefined4 *)0x0) {
          FreeAndNull(&DAT_0080ed12);
        }
        DAT_0080ed12 = (undefined4 *)Library::DKW::LIB::FUN_006aac70(0x8db);
        puVar12 = &DAT_0080bae8;
        puVar21 = DAT_0080ed12;
        for (iVar9 = 0x236; iVar9 != 0; iVar9 = iVar9 + -1) {
          *puVar21 = *puVar12;
          puVar12 = puVar12 + 1;
          puVar21 = puVar21 + 1;
        }
        *(undefined2 *)puVar21 = *(undefined2 *)puVar12;
        *(undefined1 *)((int)puVar21 + 2) = *(undefined1 *)((int)puVar12 + 2);
        DAT_0080879c = 0;
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      case CASE_4:
        SwitchOptPanel(this_00,-1);
        this_00->field_019C = 0x6102;
        if (DAT_00808783 == '\x03') {
          DAT_0080c50a = 1;
        }
        if (DAT_008067a0 == '\0') {
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        if (g_playSystem_00802A38 != (STPlaySystemC *)0x0) {
          uVar36 = (g_playSystem_00802A38->field_00E4 - g_playSystem_00802A38->field_0034 < 6000) +
                   2;
          goto LAB_00535271;
        }
        goto LAB_0053526f;
      case CASE_5:
        SwitchOptPanel(this_00,-1);
        this_00->field_019C = 0x7102;
        if (DAT_00808783 == '\x03') {
          DAT_0080c50a = 1;
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        break;
      case CASE_6:
        uVar11 = 0xffffffff;
        pcVar20 = &DAT_00807680;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = &this_00->field_0x6c;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        uVar11 = 0xffffffff;
        pcVar20 = PTR_s_SAVEGAME__0079acec;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = &this_00->field_0x6c;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        uVar11 = 0xffffffff;
        pcVar20 = (char *)&DAT_00807ddd;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = &this_00->field_0x6c;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        FUN_006b8280(&this_00->field_0x6c,&this_00->field_0x6c);
        uVar11 = 0xffffffff;
        pcVar20 = PTR_DAT_0079ad00;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = &this_00->field_0x6c;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        uVar11 = 0xffffffff;
        pcVar20 = &this_00->field_0x1f5;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = &this_00->field_0x6c;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        uVar11 = 0xffffffff;
        pcVar20 = PTR_DAT_0079acf4;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = &this_00->field_0x6c;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        DeleteFileA(&this_00->field_0x6c);
        uVar11 = 0xffffffff;
        pcVar20 = &DAT_00807680;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = &this_00->field_0x6c;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        uVar11 = 0xffffffff;
        pcVar20 = PTR_s_SAVEGAME__0079acec;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = &this_00->field_0x6c;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        uVar11 = 0xffffffff;
        pcVar20 = (char *)&DAT_00807ddd;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = &this_00->field_0x6c;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        FUN_006b8280(&this_00->field_0x6c,&this_00->field_0x6c);
        uVar11 = 0xffffffff;
        pcVar20 = PTR_DAT_0079ad00;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = &this_00->field_0x6c;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        uVar11 = 0xffffffff;
        pcVar20 = &this_00->field_0x1f5;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = &this_00->field_0x6c;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        uVar11 = 0xffffffff;
        pcVar20 = PTR_DAT_0079acf8;
        do {
          pcVar23 = pcVar20;
          if (uVar11 == 0) break;
          uVar11 = uVar11 - 1;
          pcVar23 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar23;
        } while (cVar2 != '\0');
        uVar11 = ~uVar11;
        iVar9 = -1;
        pcVar20 = &this_00->field_0x6c;
        do {
          pcVar22 = pcVar20;
          if (iVar9 == 0) break;
          iVar9 = iVar9 + -1;
          pcVar22 = pcVar20 + 1;
          cVar2 = *pcVar20;
          pcVar20 = pcVar22;
        } while (cVar2 != '\0');
        pcVar20 = pcVar23 + -uVar11;
        pcVar23 = pcVar22 + -1;
        for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
          *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
          pcVar20 = pcVar20 + 4;
          pcVar23 = pcVar23 + 4;
        }
        for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
          *pcVar23 = *pcVar20;
          pcVar20 = pcVar20 + 1;
          pcVar23 = pcVar23 + 1;
        }
        DeleteFileA(&this_00->field_0x6c);
        OVar3 = this_00->field_01A9;
        this_00->field_01A9 = this_00->field_01AA;
        this_00->field_01A4 = OVar3;
        SetOptControls(this_00);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      case CASE_7:
        this_00->field_0028 = 0x44ff;
        (*g_playSystem_00802A38->vtable->GetMessage)
                  (g_playSystem_00802A38,(int)&this_00->field_0x18);
        this_00->field_0028 = 0xc001;
        this_00->field_0034 = 0;
        /* ST_PSEUDO[raw_indirect_call]: expected typed vtable/callback call with explicit __thiscall receiver */
        (**(code **)this_00->field_0000)(&this_00->field_0x18);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      break;
    /* ST_PSEUDO[packed_or_unaligned_piece]: expected named packed member, bit extract/compose, or unaligned load */
    case CASE_6:
      DAT_00807300._1_1_ = DAT_00807300._1_1_ ^ 1;
      thunk_FUN_00568010(&g_sound,CASE_1);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_7:
      thunk_FUN_005713b0(0x807620);
      iVar9 = *(int *)(message->arg0).ptr;
      if (iVar9 == 1) {
        /* ST_PSEUDO[packed_or_unaligned_piece]: expected named packed member, bit extract/compose, or unaligned load */
        DAT_00807300._0_1_ = '\x01';
      }
      else if (iVar9 == 2) {
        /* ST_PSEUDO[packed_or_unaligned_piece]: expected named packed member, bit extract/compose, or unaligned load */
        DAT_00807300._0_1_ = '\b';
      }
      else if (iVar9 == 3) {
        /* ST_PSEUDO[packed_or_unaligned_piece]: expected named packed member, bit extract/compose, or unaligned load */
        DAT_00807300._0_1_ = '\x02';
      }
      switch((char)DAT_00807300) {
      case '\x01':
        local_c = (cMf32 *)0x0;
        break;
      case '\x02':
        local_d0.previous = g_currentExceptionFrame;
        g_currentExceptionFrame = &local_d0;
        iVar9 = Library::MSVCRT::__setjmp3(local_d0.jumpBuffer,0);
        if (iVar9 == 0) {
          if (g_sound == 0) {
            uVar11 = 0;
          }
          else {
            uVar11 = PTR_0080765c->field_0080 & 1;
          }
          if (uVar11 != 0) {
            Library::DKW::SND::FUN_006b7e00((int)PTR_0080765c,3,&local_30,&local_28);
          }
          uVar11 = (uint)(local_30 + local_28) >> 1;
        }
        else {
          uVar11 = 0;
        }
        if (g_sound == 0) {
          uVar16 = 0;
        }
        else {
          uVar16 = PTR_0080765c->field_0080 & 1;
        }
        g_currentExceptionFrame = local_d0.previous;
        if (uVar16 == 0) {
LAB_005350ba:
          uVar16 = 0;
        }
        else {
          uVar16 = (PTR_0080765c->field_0094 - PTR_0080765c->field_0090) + 1;
        }
        goto LAB_005350bc;
      default:
        break;
      case '\x04':
        local_224.previous = g_currentExceptionFrame;
        g_currentExceptionFrame = &local_224;
        iVar9 = Library::MSVCRT::__setjmp3(local_224.jumpBuffer,0);
        if (iVar9 == 0) {
          if (g_sound == 0) {
            uVar11 = 0;
          }
          else {
            uVar11 = PTR_0080765c->field_0068 & 1;
          }
          if (uVar11 != 0) {
            Library::DKW::SND::FUN_006b7e00((int)PTR_0080765c,2,&local_24,&local_2c);
          }
          uVar11 = local_24 + local_2c >> 1;
        }
        else {
          uVar11 = 0;
        }
        if (g_sound == 0) {
          uVar16 = 0;
        }
        else {
          uVar16 = PTR_0080765c->field_0068 & 1;
        }
        g_currentExceptionFrame = local_224.previous;
        if (uVar16 == 0) goto LAB_005350ba;
        uVar16 = (PTR_0080765c->field_007C - PTR_0080765c->field_0078) + 1;
LAB_005350bc:
        local_c = (cMf32 *)((uVar11 * 0x1f) / uVar16);
        this_00 = local_10;
        break;
      case '\b':
        local_c = (cMf32 *)(((DAT_0080730a + 4000) * 0x1f) / 0xfa1);
      }
      pcVar14 = local_c;
      PaintVolume(this_00,0,(byte)local_c);
      this_00->field_0028 = 0x22;
      *(cMf32 **)&this_00->field_0x2c = pcVar14;
      FUN_006e6080(this_00,2,this_00->field_01C5,(undefined4 *)&this_00->field_0x18);
      this_00->field_0028 = 0x20;
      *(uint *)&this_00->field_0x2c = (uint)((char)DAT_00807300 != '\x01');
      FUN_006e6080(this_00,2,this_00->field_01C5,(undefined4 *)&this_00->field_0x18);
      thunk_FUN_00571320(&DAT_00807620,HWND_00856d78);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_8:
      DAT_0080733b = 4 - *(char *)(message->arg0).ptr;
      if (DAT_0080877e == '\0') {
        iVar9 = (uint)DAT_0080733b * 0x18;
        DAT_00807654 = *(undefined4 *)(s_FrmPanelTy__GetMessage_007c2ae0 + iVar9 + 0x14);
      }
      else {
        iVar9 = (uint)DAT_0080733b * 0x18;
        DAT_00807654 = *(undefined4 *)(s_FrmPanelTy__GetMessage_007c2ae0 + iVar9 + 8);
      }
      local_44 = (uint)DAT_0080733b;
      if (DAT_00808783 == '\x03') {
        local_48 = 4;
        thunk_FUN_0054edf0((undefined4 *)0x5,&local_48,1,0xffffffff);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      if (PTR_00807598 != (AnonShape_GLOBAL_00807598_0C6808FB *)0x0) {
        FUN_006e8640(PTR_00807598,*(char **)(s_FrmPanelTy__GetMessage_007c2ae0 + iVar9 + 0x10),
                     *(char **)(s_FrmPanelTy__GetMessage_007c2ae0 + iVar9 + 4));
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      break;
    case CASE_9:
      DAT_0080731a = (uint)(DAT_0080731a == 0);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_D:
      this_00->field_0305 = *(char *)(message->arg0).ptr + -1;
      this_00->field_0028 = 0x20;
      *(undefined4 *)&this_00->field_0x2c = 1;
      FUN_006e6080(this_00,2,this_00->field_01B9,(undefined4 *)&this_00->field_0x18);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    }
    break;
  case MESS_BEHPANELTY_C0A0:
    OVar3 = this_00->field_01A4;
    switch(OVar3) {
    case CASE_1:
      this_00->field_01A9 = OVar3;
      this_00->field_01A4 = CASE_4;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_2:
      this_00->field_01AA = this_00->field_01A9;
      this_00->field_01A9 = OVar3;
      this_00->field_01A4 = CASE_6;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_3:
switchD_00535190_caseD_3:
      this_00->field_0028 = 0xc0a1;
      this_00->field_0034 = 0;
      /* ST_PSEUDO[raw_indirect_call]: expected typed vtable/callback call with explicit __thiscall receiver */
      (**(code **)this_00->field_0000)(&this_00->field_0x18);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    default:
      goto cf_common_exit_00536FB7;
    case CASE_5:
    case CASE_B:
      if (this_00->field_01A9 != 0) {
        this_00->field_01A4 = this_00->field_01A9;
        this_00->field_01A9 = this_00->field_01AA;
        SetOptControls(this_00);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      if (OVar3 == CASE_5) {
        this_00->field_01A4 = CASE_1;
        SetOptControls(this_00);
        SwitchOptPanel(this_00,-1);
        if (this_00->field_01AB != CASE_7) {
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        DAT_00808788 = 0;
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      SwitchOptPanel(this_00,-1);
      if (this_00->field_01AB < CASE_A) {
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      if (CASE_B < this_00->field_01AB) {
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      if (DAT_008067a0 == '\0') {
        this_00->field_019C = 0x6102;
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      break;
    /* ST_PSEUDO[packed_or_unaligned_piece]: expected named packed member, bit extract/compose, or unaligned load */
    case CASE_6:
      DAT_00807300._1_1_ = DAT_00807300._1_1_ ^ 2;
      thunk_FUN_00568010(&g_sound,CASE_2);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_8:
      DAT_0080733a = '\x03' - *(char *)(message->arg0).ptr;
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_9:
      DAT_00807322 = (uint)(DAT_00807322 == 0);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_D:
      DAT_00807348 = this_00->field_0305;
      SwitchOptPanel(this_00,-1);
      this_00->field_019C = 0x6107;
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    }
LAB_0053526f:
    uVar36 = 4;
LAB_00535271:
    CFsgsConnection::GameResult((CFsgsConnection *)&DAT_00802a90,uVar36);
    this_00->field_019C = 0x610a;
    g_currentExceptionFrame = local_8c.previous;
    return 0;
  case MESS_FSGSTY_C0A1:
    OVar3 = this_00->field_01A4;
    switch(OVar3) {
    case CASE_1:
      this_00->field_01A5 = 0x3e83;
      this_00->field_01A9 = OVar3;
      this_00->field_01AB = CASE_3;
      this_00->field_01A4 = CASE_5;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_2:
      uVar8 = this_00->field_01A9;
      this_00->field_01A9 = OVar3;
      this_00->field_01AA = uVar8;
      this_00->field_01A4 = CASE_8;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_3:
      this_00->field_0028 = 0x32;
      FUN_006e6080(this_00,2,this_00->field_01B9,(undefined4 *)&this_00->field_0x18);
      if ((int)this_00->field_01F1->elementSize < 1) {
        pcVar20 = (char *)0x0;
      }
      else {
        pcVar20 = *(char **)this_00->field_01F1->growCapacity;
      }
      uVar11 = 0xffffffff;
      do {
        pcVar23 = pcVar20;
        if (uVar11 == 0) break;
        uVar11 = uVar11 - 1;
        pcVar23 = pcVar20 + 1;
        cVar2 = *pcVar20;
        pcVar20 = pcVar23;
      } while (cVar2 != '\0');
      uVar11 = ~uVar11;
      puVar1 = &this_00->field_0x1f5;
      pcVar20 = pcVar23 + -uVar11;
      pcVar23 = puVar1;
      for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
        *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
        pcVar20 = pcVar20 + 4;
        pcVar23 = pcVar23 + 4;
      }
      for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
        *pcVar23 = *pcVar20;
        pcVar20 = pcVar20 + 1;
        pcVar23 = pcVar23 + 1;
      }
      pbVar24 = &DAT_008016a0;
      pbVar15 = puVar1;
      do {
        bVar28 = *pbVar15;
        bVar25 = bVar28 < *pbVar24;
        if (bVar28 != *pbVar24) {
LAB_0053556a:
          iVar9 = (1 - (uint)bVar25) - (uint)(bVar25 != 0);
          goto LAB_0053556f;
        }
        if (bVar28 == 0) break;
        bVar28 = pbVar15[1];
        bVar25 = bVar28 < pbVar24[1];
        if (bVar28 != pbVar24[1]) goto LAB_0053556a;
        pbVar15 = pbVar15 + 2;
        pbVar24 = pbVar24 + 2;
      } while (bVar28 != 0);
      iVar9 = 0;
LAB_0053556f:
      if (iVar9 == 0) {
        this_00->field_01A5 = 0x3f4b;
        this_00->field_01AA = this_00->field_01A9;
        this_00->field_01A9 = this_00->field_01A4;
        this_00->field_01A4 = CASE_B;
        SetOptControls(this_00);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      iVar9 = CheckFiles(this_00,puVar1);
      if (iVar9 != 0) {
        this_00->field_01AA = this_00->field_01A9;
        this_00->field_01A5 = 0x3f49;
        this_00->field_01A9 = this_00->field_01A4;
        this_00->field_01A4 = CASE_5;
        this_00->field_01AB = CASE_1;
        SetOptControls(this_00);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
switchD_00534835_caseD_1:
      thunk_FUN_005335e0();
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_4:
      this_00->field_0028 = 0x26;
      FUN_006e6080(this_00,2,this_00->field_01B5,(undefined4 *)&this_00->field_0x18);
      if (*(int *)&this_00->field_0x2c < (int)this_00->field_01E9->elementSize) {
        pcVar20 = *(char **)(this_00->field_01E9->growCapacity + *(int *)&this_00->field_0x2c * 4);
      }
      else {
        pcVar20 = (char *)0x0;
      }
      uVar11 = 0xffffffff;
      do {
        pcVar23 = pcVar20;
        if (uVar11 == 0) break;
        uVar11 = uVar11 - 1;
        pcVar23 = pcVar20 + 1;
        cVar2 = *pcVar20;
        pcVar20 = pcVar23;
      } while (cVar2 != '\0');
      uVar11 = ~uVar11;
      pcVar20 = pcVar23 + -uVar11;
      pcVar23 = &this_00->field_0x1f5;
      for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
        *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
        pcVar20 = pcVar20 + 4;
        pcVar23 = pcVar23 + 4;
      }
      for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
        *pcVar23 = *pcVar20;
        pcVar20 = pcVar20 + 1;
        pcVar23 = pcVar23 + 1;
      }
      this_00->field_01E1 = *(undefined4 *)&this_00->field_0x2c;
      this_00->field_01A5 = 0x3f48;
      this_00->field_01AA = this_00->field_01A9;
      this_00->field_01A9 = this_00->field_01A4;
      this_00->field_01AB = CASE_2;
      this_00->field_01A4 = CASE_5;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    /* ST_PSEUDO[packed_or_unaligned_piece]: expected named packed member, bit extract/compose, or unaligned load */
    case CASE_6:
      DAT_00807300._1_1_ = DAT_00807300._1_1_ ^ 4;
      thunk_FUN_00568010(&g_sound,CASE_4);
      SoundClassTy::PlaySound_thunk
                ((SoundClassTy *)&g_sound,SOUND_MODE_1,(char *)0x0,0x4b7,(SoundPosition *)0x0,0);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_9:
      DAT_0080731e = (uint)(DAT_0080731e == 0);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_D:
      DAT_00807342 = (uint)(DAT_00807342 == 0);
      if (g_popUp_008016D8 != (PopUpTy *)0x0) {
        PopUpTy::ChangeState(g_popUp_008016D8);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      break;
    case CASE_E:
      local_c = (cMf32 *)0x0;
      local_14 = (undefined4 *)0x0;
      local_20 = (ushort *)0x0;
      local_8 = 1;
      local_1c = (uint *)0x0;
      this_00->field_0028 = 0x26;
      FUN_006e6080(this_00,2,this_00->field_01B5,(undefined4 *)&this_00->field_0x18);
      pDVar6 = this_00->field_02FD;
      if (*(uint *)&this_00->field_0x2c < pDVar6->count) {
        /* ST_PSEUDO[dynamic_array_indexing]: expected DArrayAt<T>(pDVar6, *(uint *)&this_00->field_0x2c) (runtime stride) */
        pvVar17 = (void *)(pDVar6->elementSize * *(uint *)&this_00->field_0x2c + (int)pDVar6->data);
      }
      else {
        pvVar17 = (void *)0x0;
      }
      if (*(int *)((int)pvVar17 + 0x6c) == 0) {
        switch(_DAT_008087a0 & 0xff) {
        case 1:
        case 4:
        case 6:
        case 7:
        case 0xd:
        case 0xe:
        case 0x13:
          wsprintfA((LPSTR)&DAT_0080f33a,"%s%s%s",&DAT_00807680,PTR_s_SYSTEM__0079acf0,
                    PTR_s_STRATEGS_0079acfc);
          break;
        case 2:
        case 3:
        case 5:
        case 8:
        case 0xf:
          wsprintfA((LPSTR)&DAT_0080f33a,"%s%s",&DAT_00807680,&DAT_0080ef1e);
        }
        local_19c.previous = g_currentExceptionFrame;
        g_currentExceptionFrame = &local_19c;
        iVar9 = Library::MSVCRT::__setjmp3(local_19c.jumpBuffer,0);
        this_00 = local_10;
        pcVar14 = local_c;
        if (iVar9 == 0) {
          pcVar14 = (cMf32 *)Library::Ourlib::MF32INT::FUN_006f0ec0
                                       (0x345,(byte *)&DAT_0080f33a,0,0,0);
          pDVar6 = this_00->field_02FD;
          if (*(uint *)&this_00->field_0x2c < pDVar6->count) {
            /* ST_PSEUDO[dynamic_array_indexing]: expected DArrayAt<T>(pDVar6, *(uint *)&this_00->field_0x2c) (runtime stride) */
            pvVar17 = (void *)(pDVar6->elementSize * *(uint *)&this_00->field_0x2c +
                              (int)pDVar6->data);
          }
          else {
            pvVar17 = (void *)0x0;
          }
          local_c = pcVar14;
          local_20 = LoadStrategData((int)pcVar14,(char *)((int)pvVar17 + 0x4c),(int *)&local_18);
          uVar11 = local_8;
          if (local_20 != (ushort *)0x0) {
            local_14 = Library::DKW::LIB::FUN_006aac10(local_18 + local_8 * 2);
            *(undefined1 *)local_14 = 1;
            pbVar15 = (byte *)((int)local_14 + uVar11);
            *pbVar15 = DAT_0080874d;
            puVar18 = local_20;
            pbVar15 = pbVar15 + uVar11;
            for (uVar16 = local_18 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
              *(undefined4 *)pbVar15 = *(undefined4 *)puVar18;
              puVar18 = puVar18 + 2;
              pbVar15 = pbVar15 + 4;
            }
            for (uVar11 = local_18 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
              *pbVar15 = (byte)*puVar18;
              puVar18 = (ushort *)((int)puVar18 + 1);
              pbVar15 = pbVar15 + 1;
            }
            thunk_FUN_0054edf0((undefined4 *)0x32,local_14,1,local_18 + local_8 * 2);
            local_1c = (uint *)0x1;
            pcVar14 = local_c;
          }
        }
        g_currentExceptionFrame = local_19c.previous;
        if (pcVar14 != (cMf32 *)0x0) {
          cMf32::delete(pcVar14);
        }
        if (local_20 != (ushort *)0x0) {
          FreeAndNull(&local_20);
        }
        if (local_14 != (undefined4 *)0x0) {
          FreeAndNull(&local_14);
        }
      }
      else {
        local_14 = Library::DKW::LIB::FUN_006aac10(2);
        if (local_14 != (undefined4 *)0x0) {
          *(undefined1 *)local_14 = 0;
          *(byte *)((int)local_14 + 1) = DAT_0080874d;
          thunk_FUN_0054edf0((undefined4 *)0x32,local_14,1,2);
          FreeAndNull(&local_14);
          local_1c = (uint *)0x1;
        }
      }
      if (local_1c != (uint *)0x0) {
        this_00->field_0028 = 0xc001;
        this_00->field_0034 = 0;
        /* ST_PSEUDO[raw_indirect_call]: expected typed vtable/callback call with explicit __thiscall receiver */
        (**(code **)this_00->field_0000)(&this_00->field_0x18);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
    }
    break;
  case MESS_SHARED_C0A2:
    OVar3 = this_00->field_01A4;
    switch(OVar3) {
    case CASE_1:
      if (DAT_008067a0 == '\0') {
        iVar9 = 0x3e84;
      }
      else {
        iVar9 = 0x3e96 - (uint)(g_playSystem_00802A38->field_00E4 -
                                g_playSystem_00802A38->field_0034 < 6000);
      }
      this_00->field_01A9 = OVar3;
      this_00->field_01A5 = iVar9;
      this_00->field_01AB = CASE_4;
      this_00->field_01A4 = CASE_5;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_2:
      uVar8 = this_00->field_01A9;
      this_00->field_01A9 = OVar3;
      this_00->field_01AA = uVar8;
      this_00->field_01A4 = CASE_9;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_3:
    case CASE_4:
      this_00->field_0028 = 0x26;
      FUN_006e6080(this_00,2,this_00->field_01B5,(undefined4 *)&this_00->field_0x18);
      if (*(int *)&this_00->field_0x2c < (int)this_00->field_01E9->elementSize) {
        pcVar20 = *(char **)(this_00->field_01E9->growCapacity + *(int *)&this_00->field_0x2c * 4);
      }
      else {
        pcVar20 = (char *)0x0;
      }
      uVar11 = 0xffffffff;
      do {
        pcVar23 = pcVar20;
        if (uVar11 == 0) break;
        uVar11 = uVar11 - 1;
        pcVar23 = pcVar20 + 1;
        cVar2 = *pcVar20;
        pcVar20 = pcVar23;
      } while (cVar2 != '\0');
      uVar11 = ~uVar11;
      pcVar20 = pcVar23 + -uVar11;
      pcVar23 = &this_00->field_0x1f5;
      for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
        *(undefined4 *)pcVar23 = *(undefined4 *)pcVar20;
        pcVar20 = pcVar20 + 4;
        pcVar23 = pcVar23 + 4;
      }
      for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
        *pcVar23 = *pcVar20;
        pcVar20 = pcVar20 + 1;
        pcVar23 = pcVar23 + 1;
      }
      this_00->field_01E1 = *(undefined4 *)&this_00->field_0x2c;
      this_00->field_01A5 = 0x3f4a;
      this_00->field_01AA = this_00->field_01A9;
      this_00->field_01A9 = this_00->field_01A4;
      this_00->field_01AB = CASE_6;
      this_00->field_01A4 = CASE_5;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    /* ST_PSEUDO[packed_or_unaligned_piece]: expected named packed member, bit extract/compose, or unaligned load */
    case CASE_6:
      DAT_00807300._1_1_ = DAT_00807300._1_1_ ^ 8;
      thunk_FUN_00568010(&g_sound,CASE_8);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_8:
      g_currentExceptionFrame = local_8c.previous;
      DAT_00807349 = *(char *)(message->arg0).ptr + -1;
      return 0;
    case CASE_9:
      DAT_00807326 = DAT_00807326 == '\0';
      thunk_FUN_00440700((uint)(byte)DAT_00807326);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_D:
      DAT_00807346 = (*(char *)&message->arg0 + '\x01') * '\x05';
      DibPut((AnonShape_006B5B10_E0D06CF1 *)this_00->field_0068,0xaa,0x6f,'\x01',
             (byte *)this_00->field_018C);
      wsprintfA(&this_00->field_0x6c,"%d",(uint)(byte)DAT_00807346);
      ccFntTy::SetSurf(this_00->field_017C,this_00->field_0068,0,0xaa,0x6f,
                       *(int *)(this_00->field_018C + 2),*(int *)(this_00->field_018C + 4));
      ccFntTy::WrTxt(this_00->field_017C,(uint *)&this_00->field_0x6c,-2,-1,0,-1,-1);
      if (g_popUp_008016D8 != (PopUpTy *)0x0) {
        PopUpTy::ChangeNumStr(g_popUp_008016D8);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
    }
    break;
  case MESS_SHARED_C0A3:
    OVar3 = this_00->field_01A4;
    switch(OVar3) {
    case CASE_1:
      this_00->field_01A5 = 0x3e85;
      this_00->field_01A9 = OVar3;
      this_00->field_01AB = CASE_5;
      this_00->field_01A4 = CASE_5;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_2:
      uVar8 = this_00->field_01A9;
      this_00->field_01A9 = OVar3;
      this_00->field_01AA = uVar8;
      this_00->field_01A4 = CASE_D;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_6:
      DAT_0080730e = ((message->arg0).u32 * 4000) / 0x1e - 4000;
      thunk_FUN_005682c0(&g_sound,DAT_0080730e);
      PaintVolume(this_00,0,*(byte *)&message->arg0);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_7:
      if ((char)DAT_00807300 == '\x02') {
        if (g_sound == 0) {
          uVar11 = 0;
        }
        else {
          uVar11 = PTR_0080765c->field_0080 & 1;
        }
        if (uVar11 != 0) {
          if (g_sound == 0) {
            uVar11 = 0;
          }
          else {
            uVar11 = PTR_0080765c->field_0080 & 1;
          }
          if (uVar11 == 0) {
            iVar9 = 0;
          }
          else {
            iVar9 = (PTR_0080765c->field_0094 - PTR_0080765c->field_0090) + 1;
          }
          local_8 = ((iVar9 + -1) * (message->arg0).u32) / 0x1e;
          /* ST_PSEUDO[packed_or_unaligned_piece]: expected named packed member, bit extract/compose, or unaligned load */
          DAT_00807300._2_2_ = (undefined2)local_8;
          uRam00807304 = (undefined2)(local_8 >> 0x10);
          local_158.previous = g_currentExceptionFrame;
          g_currentExceptionFrame = &local_158;
          iVar9 = Library::MSVCRT::__setjmp3(local_158.jumpBuffer,0);
          pIVar7 = local_158.previous;
          if (iVar9 == 0) {
            if (g_sound == 0) {
              uVar11 = 0;
            }
            else {
              uVar11 = PTR_0080765c->field_0080 & 1;
            }
            if (uVar11 != 0) {
              Library::DKW::SND::FUN_006b7f00((int)PTR_0080765c,3,local_8,local_8);
            }
            g_currentExceptionFrame = local_158.previous;
            pIVar7 = g_currentExceptionFrame;
          }
LAB_00535ef1:
          g_currentExceptionFrame = pIVar7;
          PaintVolume(local_10,0,*(byte *)&message->arg0);
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
      }
      else if ((char)DAT_00807300 == '\x04') {
        if (g_sound == 0) {
          uVar11 = 0;
        }
        else {
          uVar11 = PTR_0080765c->field_0068 & 1;
        }
        if (uVar11 != 0) {
          if (g_sound == 0) {
            uVar11 = 0;
          }
          else {
            uVar11 = PTR_0080765c->field_0068 & 1;
          }
          if (uVar11 == 0) {
            iVar9 = 0;
          }
          else {
            iVar9 = (PTR_0080765c->field_007C - PTR_0080765c->field_0078) + 1;
          }
          _DAT_00807306 = ((iVar9 + -1) * (message->arg0).u32) / 0x1e;
          local_114.previous = g_currentExceptionFrame;
          g_currentExceptionFrame = &local_114;
          local_8 = _DAT_00807306;
          iVar9 = Library::MSVCRT::__setjmp3(local_114.jumpBuffer,0);
          pIVar7 = local_114.previous;
          if (iVar9 == 0) {
            if (g_sound == 0) {
              uVar11 = 0;
            }
            else {
              uVar11 = PTR_0080765c->field_0068 & 1;
            }
            if (uVar11 != 0) {
              Library::DKW::SND::FUN_006b7f00((int)PTR_0080765c,2,local_8,local_8);
            }
            g_currentExceptionFrame = local_114.previous;
            pIVar7 = g_currentExceptionFrame;
          }
          goto LAB_00535ef1;
        }
      }
      else if ((char)DAT_00807300 == '\b') {
        DAT_0080730a = ((message->arg0).u32 * 4000) / 0x1e - 4000;
        thunk_FUN_005686c0(&g_sound,DAT_0080730a);
        PaintVolume(this_00,0,*(byte *)&message->arg0);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      break;
    case CASE_9:
      DAT_00807328 = (uint)(DAT_00807328 == 0);
      if (DAT_00807328 != 0) {
        FUN_006ddb70((AnonShape_006DDB70_C8239555 *)PTR_00807598);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      FUN_006ddbb0(PTR_00807598);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_D:
      DAT_00807347 = *(byte *)&message->arg0;
      DibPut((AnonShape_006B5B10_E0D06CF1 *)this_00->field_0068,0xaa,0x81,'\x01',
             (byte *)this_00->field_018C);
      wsprintfA(&this_00->field_0x6c,"%d",DAT_00807347 + 1);
      ccFntTy::SetSurf(this_00->field_017C,this_00->field_0068,0,0xaa,0x81,
                       *(int *)(this_00->field_018C + 2),*(int *)(this_00->field_018C + 4));
      ccFntTy::WrTxt(this_00->field_017C,(uint *)&this_00->field_0x6c,-2,-1,0,-1,-1);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    }
    break;
  case MESS_BLDLABPANELTY_C0A4:
    if (this_00->field_01A4 == CASE_1) {
      this_00->field_01A9 = 1;
      this_00->field_01A4 = CASE_C;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    }
    if (this_00->field_01A4 == CASE_9) {
      DAT_0080732c = (uint)(DAT_0080732c == 0);
      this_00->field_0028 = 0x440f;
      FUN_006e6000(this_00,0xf,0,(undefined4 *)&this_00->field_0x18);
      if (PTR_00807598 != (AnonShape_GLOBAL_00807598_0C6808FB *)0x0) {
        FUN_006e8630(PTR_00807598,DAT_0080732c);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
    }
    break;
  case MESS_FRMPANELTY_C0A5:
    OVar3 = this_00->field_01A4;
    if (OVar3 == CASE_1) {
      this_00->field_01A9 = 1;
      this_00->field_01A4 = CASE_2;
      SetOptControls(this_00);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    }
    if ((CASE_5 < OVar3) && (OVar3 < CASE_8)) {
      if (g_sound == 0) {
        uVar11 = 0;
      }
      else {
        uVar11 = PTR_0080765c->field_0050 & 1;
      }
      if (uVar11 != 0) {
        if (g_sound == 0) {
          uVar11 = 0;
        }
        else {
          uVar11 = PTR_0080765c->field_0050 & 1;
        }
        if (uVar11 == 0) {
          iVar9 = 0;
        }
        else {
          iVar9 = (PTR_0080765c->field_0064 - PTR_0080765c->field_0060) + 1;
        }
        _DAT_00807312 = ((iVar9 + -1) * (message->arg0).u32) / 0x1e;
        local_1e0.previous = g_currentExceptionFrame;
        g_currentExceptionFrame = &local_1e0;
        local_8 = _DAT_00807312;
        iVar9 = Library::MSVCRT::__setjmp3(local_1e0.jumpBuffer,0);
        if (iVar9 == 0) {
          if (g_sound == 0) {
            uVar11 = 0;
          }
          else {
            uVar11 = PTR_0080765c->field_0050 & 1;
          }
          if (uVar11 != 0) {
            Library::DKW::SND::FUN_006b7f00((int)PTR_0080765c,1,local_8,local_8);
          }
        }
        g_currentExceptionFrame = local_1e0.previous;
        PaintVolume(local_10,2,*(byte *)&message->arg0);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
    }
    break;
  case MESS_BLDLABPANELTY_C0A4|MESS_ID_CREATE:
    if ((CASE_5 < this_00->field_01A4) && (this_00->field_01A4 < CASE_8)) {
      if (g_sound == 0) {
        uVar11 = 0;
      }
      else {
        uVar11 = PTR_0080765c->field_0038 & 1;
      }
      if (uVar11 != 0) {
        if (g_sound == 0) {
          uVar11 = 0;
        }
        else {
          uVar11 = PTR_0080765c->field_0038 & 1;
        }
        if (uVar11 == 0) {
          iVar9 = 0;
        }
        else {
          iVar9 = (PTR_0080765c->field_004C - PTR_0080765c->field_0048) + 1;
        }
        _DAT_00807316 = ((iVar9 + -1) * (message->arg0).u32) / 0x1e;
        local_268.previous = g_currentExceptionFrame;
        g_currentExceptionFrame = &local_268;
        local_8 = _DAT_00807316;
        iVar9 = Library::MSVCRT::__setjmp3(local_268.jumpBuffer,0);
        if (iVar9 == 0) {
          if (g_sound == 0) {
            uVar11 = 0;
          }
          else {
            uVar11 = PTR_0080765c->field_0038 & 1;
          }
          if (uVar11 != 0) {
            Library::DKW::SND::FUN_006b7f00((int)PTR_0080765c,0,local_8,local_8);
          }
        }
        g_currentExceptionFrame = local_268.previous;
        PaintVolume(local_10,3,*(byte *)&message->arg0);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
    }
    break;
  case MESS_INTERCOMPANELTY_C0AF:
    switch(this_00->field_01A4) {
    case CASE_1:
      if (DAT_0080874e == '\x03') {
        pcVar35 = thunk_FUN_0052a280;
        UVar34 = 0x3e81;
      }
      else {
        pcVar35 = thunk_FUN_0052a1e0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a230;
        }
        UVar34 = 0x3e81;
      }
      goto cf_common_exit_00536F8B;
    case CASE_2:
      if (DAT_0080874e == '\x03') {
        pcVar31 = this_00->field_0180;
        pcVar35 = thunk_FUN_0052a280;
        UVar34 = 0x3e8b;
      }
      else {
        pcVar35 = thunk_FUN_0052a1e0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a230;
        }
        pcVar31 = this_00->field_0180;
        UVar34 = 0x3e8b;
      }
      goto cf_common_exit_00536F92;
    default:
      goto cf_common_exit_00536FB7;
    case CASE_5:
      UVar34 = 0x273e;
      goto cf_common_exit_00536DB5;
    case CASE_6:
      if (DAT_0080874e == '\x03') {
        iVar9 = 0;
        pcVar35 = thunk_FUN_0052a770;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3efc;
      }
      else {
        pcVar35 = thunk_FUN_0052a6f0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a730;
        }
        iVar9 = 0;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3efc;
      }
      break;
    case CASE_7:
      iVar9 = *(int *)(message->arg0).ptr;
      if (iVar9 == 1) {
        if (DAT_0080874e == '\x03') {
          iVar9 = 1;
          pcVar35 = thunk_FUN_0052a640;
          pcVar30 = thunk_FUN_0052a4d0;
          UVar34 = 0x3ee7;
        }
        else {
          pcVar35 = thunk_FUN_0052a540;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a5c0;
          }
          iVar9 = 1;
          pcVar30 = thunk_FUN_0052a4d0;
          UVar34 = 0x3ee7;
        }
      }
      else if (iVar9 == 2) {
        if (DAT_0080874e == '\x03') {
          iVar9 = 1;
          pcVar35 = thunk_FUN_0052a640;
          pcVar30 = thunk_FUN_0052a4d0;
          UVar34 = 0x3ee5;
        }
        else {
          pcVar35 = thunk_FUN_0052a540;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a5c0;
          }
          iVar9 = 1;
          pcVar30 = thunk_FUN_0052a4d0;
          UVar34 = 0x3ee5;
        }
      }
      else {
        if (iVar9 != 3) {
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        if (DAT_0080874e == '\x03') {
          iVar9 = 1;
          pcVar35 = thunk_FUN_0052a640;
          pcVar30 = thunk_FUN_0052a4d0;
          UVar34 = 0x3ee6;
        }
        else {
          pcVar35 = thunk_FUN_0052a540;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a5c0;
          }
          iVar9 = 1;
          pcVar30 = thunk_FUN_0052a4d0;
          UVar34 = 0x3ee6;
        }
      }
      break;
    case CASE_8:
      iVar9 = *(int *)(message->arg0).ptr;
      if (iVar9 == 1) {
        if (DAT_0080874e == '\x03') {
          pcVar35 = thunk_FUN_0052a640;
        }
        else {
          pcVar35 = thunk_FUN_0052a540;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a5c0;
          }
        }
        iVar9 = 1;
        pcVar30 = thunk_FUN_0052a4d0;
        UVar34 = 0x3ef1;
        pCVar13 = thunk_FUN_00571240("BUT_BUTOPT1",0);
        PaintDblBut(this_00,(AnonShape_0052EFB0_CC661192 *)message,pCVar13,UVar34,pcVar30,pcVar35,
                    iVar9);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      if (iVar9 != 2) {
        if (iVar9 != 3) {
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        if (DAT_0080874e == '\x03') {
          pcVar35 = thunk_FUN_0052a640;
        }
        else {
          pcVar35 = thunk_FUN_0052a540;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a5c0;
          }
        }
        iVar9 = 1;
        pcVar30 = thunk_FUN_0052a4d0;
        UVar34 = 0x3ef2;
        pCVar13 = thunk_FUN_00571240("BUT_BUTOPT1",0);
        PaintDblBut(this_00,(AnonShape_0052EFB0_CC661192 *)message,pCVar13,UVar34,pcVar30,pcVar35,
                    iVar9);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      if (DAT_0080874e == '\x03') {
        pcVar35 = thunk_FUN_0052a640;
      }
      else {
joined_r0x0053686a:
        pcVar35 = thunk_FUN_0052a540;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a5c0;
        }
      }
LAB_00536434:
      UVar34 = 0x3ef0;
cf_common_exit_00536441:
      iVar9 = 1;
      pcVar30 = thunk_FUN_0052a4d0;
      pCVar13 = thunk_FUN_00571240("BUT_BUTOPT1",0);
      PaintDblBut(this_00,(AnonShape_0052EFB0_CC661192 *)message,pCVar13,UVar34,pcVar30,pcVar35,
                  iVar9);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_9:
      if (DAT_0080874e == '\x03') {
        iVar9 = 0;
        pcVar35 = thunk_FUN_0052a770;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3ef4;
      }
      else {
        pcVar35 = thunk_FUN_0052a6f0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a730;
        }
        iVar9 = 0;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3ef4;
      }
      break;
    case CASE_D:
      iVar9 = *(int *)(message->arg0).ptr;
      if (iVar9 == 1) {
        if (DAT_0080874e == '\x03') {
          pcVar35 = thunk_FUN_0052a640;
        }
        else {
          pcVar35 = thunk_FUN_0052a540;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a5c0;
          }
        }
        iVar9 = 1;
        pcVar30 = thunk_FUN_0052a4d0;
        UVar34 = 0x3e90;
        pCVar13 = thunk_FUN_00571240("BUT_BUTOPT1",0);
        PaintDblBut(this_00,(AnonShape_0052EFB0_CC661192 *)message,pCVar13,UVar34,pcVar30,pcVar35,
                    iVar9);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      if (iVar9 == 2) {
        if (DAT_0080874e == '\x03') {
          pcVar35 = thunk_FUN_0052a640;
        }
        else {
          pcVar35 = thunk_FUN_0052a540;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a5c0;
          }
        }
        iVar9 = 1;
        pcVar30 = thunk_FUN_0052a4d0;
        UVar34 = 0x3e91;
        pCVar13 = thunk_FUN_00571240("BUT_BUTOPT1",0);
        PaintDblBut(this_00,(AnonShape_0052EFB0_CC661192 *)message,pCVar13,UVar34,pcVar30,pcVar35,
                    iVar9);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      if (iVar9 == 3) {
        if (DAT_0080874e == '\x03') {
          pcVar35 = thunk_FUN_0052a640;
        }
        else {
          pcVar35 = thunk_FUN_0052a540;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a5c0;
          }
        }
        iVar9 = 1;
        pcVar30 = thunk_FUN_0052a4d0;
        UVar34 = 0x3e92;
        pCVar13 = thunk_FUN_00571240("BUT_BUTOPT1",0);
        PaintDblBut(this_00,(AnonShape_0052EFB0_CC661192 *)message,pCVar13,UVar34,pcVar30,pcVar35,
                    iVar9);
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      goto cf_common_exit_00536FB7;
    }
cf_common_exit_00536EE8:
    uVar33 = 0;
    iVar29 = 1;
    pCVar13 = thunk_FUN_00571240("BUT_BUTOPT",0);
    pCVar13 = FUN_006f2c00(pCVar13,iVar29,uVar33);
    PaintDblBut(this_00,(AnonShape_0052EFB0_CC661192 *)message,pCVar13,UVar34,pcVar30,pcVar35,iVar9)
    ;
    g_currentExceptionFrame = local_8c.previous;
    return 0;
  case 0xc0b0:
    switch(this_00->field_01A4) {
    case CASE_1:
      if (DAT_0080874e == '\x03') {
        pcVar31 = this_00->field_0180;
        pcVar35 = thunk_FUN_0052a280;
        UVar34 = 0x3e82;
      }
      else {
        pcVar35 = thunk_FUN_0052a1e0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a230;
        }
        pcVar31 = this_00->field_0180;
        UVar34 = 0x3e82;
      }
      break;
    case CASE_2:
      if (DAT_0080874e == '\x03') {
        pcVar35 = thunk_FUN_0052a280;
        UVar34 = 0x3e8a;
      }
      else {
        pcVar35 = thunk_FUN_0052a1e0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a230;
        }
        UVar34 = 0x3e8a;
      }
      goto cf_common_exit_00536F8B;
    case CASE_3:
      puVar18 = this_00->field_01E5;
      uVar11 = *(uint *)(puVar18 + 10);
      if (uVar11 == 0) {
        uVar11 = ((uint)puVar18[7] * *(int *)(puVar18 + 2) + 0x1f >> 3 & 0x1ffffffc) *
                 *(int *)(puVar18 + 4);
      }
      local_24 = (DAT_0080874e != '\x03') - 1 & 0x73;
      puVar12 = (undefined4 *)FUN_006b4fa0((int)puVar18);
      uVar8 = (undefined1)local_24;
      for (uVar16 = uVar11 >> 2; uVar16 != 0; uVar16 = uVar16 - 1) {
        *puVar12 = CONCAT22(CONCAT11(uVar8,uVar8),CONCAT11(uVar8,uVar8));
        puVar12 = puVar12 + 1;
      }
      for (uVar11 = uVar11 & 3; uVar11 != 0; uVar11 = uVar11 - 1) {
        *(undefined1 *)puVar12 = uVar8;
        puVar12 = (undefined4 *)((int)puVar12 + 1);
      }
      ccFntTy::SetSurf(this_00->field_017C,(int)this_00->field_01E5,0,0,0,0,0);
      ccFntTy::WrStr(this_00->field_017C,*(uint **)(message->arg0).ptr,0,-1,0);
      uVar4 = (message->arg1).words.low;
      if ((uVar4 != 0xffff) || ((message->arg1).words.high != 0xffff)) {
        uVar11 = (uint)(message->arg1).words.high;
        FUN_006b5b10((AnonShape_006E6FB0_BC494FEA *)this_00->field_01E5,0,(uint)uVar4,uVar11,
                     (uint)uVar4,uVar11 + 10,0x6c,0xd);
      }
      puVar18 = this_00->field_01E5;
      Library::DKW::WGR::FUN_006b55f0
                ((AnonShape_006B5B10_E0D06CF1 *)this_00->field_0068,0,0x26,0x1c,(byte *)puVar18,0,0,
                 0,*(int *)(puVar18 + 2),*(int *)(puVar18 + 4));
      Library::DKW::DDX::FUN_006b3640
                ((int *)PTR_008075a8,this_00->field_0060,0xffffffff,this_00->field_003C,
                 this_00->field_0044);
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    default:
      goto cf_common_exit_00536FB7;
    case CASE_5:
      UVar34 = 0x273f;
      goto cf_common_exit_00536DB5;
    case CASE_6:
      if (DAT_0080874e == '\x03') {
        iVar9 = 0;
        pcVar35 = thunk_FUN_0052a770;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3efd;
      }
      else {
        pcVar35 = thunk_FUN_0052a6f0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a730;
        }
        iVar9 = 0;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3efd;
      }
      goto cf_common_exit_00536EE8;
    case CASE_8:
      iVar9 = *(int *)(message->arg0).ptr;
      if (iVar9 == 1) {
        if (DAT_0080874e == '\x03') {
          pcVar35 = thunk_FUN_0052a640;
          UVar34 = 0x3ef1;
        }
        else {
          pcVar35 = thunk_FUN_0052a540;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a5c0;
          }
          UVar34 = 0x3ef1;
        }
        goto cf_common_exit_00536441;
      }
      if (iVar9 != 2) {
        if (iVar9 != 3) {
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        if (DAT_0080874e == '\x03') {
          pcVar35 = thunk_FUN_0052a640;
          UVar34 = 0x3ef2;
          goto cf_common_exit_00536441;
        }
        goto LAB_00536d52;
      }
      if (DAT_0080874e != '\x03') goto joined_r0x0053686a;
      pcVar35 = thunk_FUN_0052a640;
      goto LAB_00536434;
    case CASE_9:
      if (DAT_0080874e == '\x03') {
        iVar9 = 0;
        pcVar35 = thunk_FUN_0052a770;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3ef5;
      }
      else {
        pcVar35 = thunk_FUN_0052a6f0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a730;
        }
        iVar9 = 0;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3ef5;
      }
      goto cf_common_exit_00536EE8;
    case CASE_B:
switchD_00536a2b_caseD_3:
      UVar34 = 0x2751;
cf_common_exit_00536DB5:
      pCVar13 = thunk_FUN_00571240("BUT_MEDIUM",0);
      UPanelTy::PaintIBut((UPanelTy *)this_00,(AnonShape_0053DAF0_3BDC2979 *)message,pCVar13,UVar34)
      ;
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    case CASE_D:
      if (DAT_0080874e == '\x03') {
        pcVar31 = this_00->field_0180;
        pcVar35 = thunk_FUN_0052a280;
        UVar34 = 0x3e93;
      }
      else {
        pcVar35 = thunk_FUN_0052a1e0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a230;
        }
        pcVar31 = this_00->field_0180;
        UVar34 = 0x3e93;
      }
    }
    goto cf_common_exit_00536F92;
  case MESS_BEHPANELTY_C0B1:
    switch(this_00->field_01A4) {
    case CASE_1:
      if (DAT_0080874e == '\x03') {
        pcVar31 = this_00->field_0180;
        pcVar35 = thunk_FUN_0052a280;
        UVar34 = 0x3e83;
      }
      else {
        pcVar35 = thunk_FUN_0052a1e0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a230;
        }
        pcVar31 = this_00->field_0180;
        UVar34 = 0x3e83;
      }
      goto cf_common_exit_00536F92;
    case CASE_2:
      if (DAT_0080874e == '\x03') {
        pcVar35 = thunk_FUN_0052a280;
        UVar34 = 0x3e8c;
      }
      else {
        pcVar35 = thunk_FUN_0052a1e0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a230;
        }
        UVar34 = 0x3e8c;
      }
      goto cf_common_exit_00536F8B;
    case CASE_3:
    case CASE_4:
      goto switchD_00536a2b_caseD_3;
    default:
      goto cf_common_exit_00536FB7;
    case CASE_6:
      if (DAT_0080874e == '\x03') {
        iVar9 = 0;
        pcVar35 = thunk_FUN_0052a770;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3efe;
      }
      else {
        pcVar35 = thunk_FUN_0052a6f0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a730;
        }
        iVar9 = 0;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3efe;
      }
      break;
    case CASE_8:
      if (*(int *)(message->arg0).ptr == 1) {
        if (DAT_0080874e == '\x03') {
          pcVar35 = thunk_FUN_0052a640;
          UVar34 = 0x3f02;
        }
        else {
          pcVar35 = thunk_FUN_0052a540;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a5c0;
          }
          UVar34 = 0x3f02;
        }
      }
      else {
        if (*(int *)(message->arg0).ptr != 2) {
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        if (DAT_0080874e == '\x03') {
          pcVar35 = thunk_FUN_0052a640;
          UVar34 = 0x3f03;
        }
        else {
          pcVar35 = thunk_FUN_0052a540;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a5c0;
          }
          UVar34 = 0x3f03;
        }
      }
      goto cf_common_exit_00536441;
    case CASE_9:
      if (DAT_0080874e == '\x03') {
        iVar9 = 0;
        pcVar35 = thunk_FUN_0052a770;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3ef6;
      }
      else {
        pcVar35 = thunk_FUN_0052a6f0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a730;
        }
        iVar9 = 0;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3ef6;
      }
      break;
    case CASE_D:
      if (DAT_0080874e == '\x03') {
        iVar9 = 0;
        pcVar35 = thunk_FUN_0052a770;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3ef8;
      }
      else {
        pcVar35 = thunk_FUN_0052a6f0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a730;
        }
        iVar9 = 0;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3ef8;
      }
      break;
    case CASE_E:
      UVar34 = 0x2752;
      goto cf_common_exit_00536DB5;
    }
    goto cf_common_exit_00536EE8;
  case 0xc0b2:
    switch(this_00->field_01A4) {
    case CASE_1:
      if (DAT_0080874e == '\x03') {
        pcVar35 = thunk_FUN_0052a280;
      }
      else {
        pcVar35 = thunk_FUN_0052a1e0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a230;
        }
      }
      if (DAT_008067a0 == '\0') {
        UVar34 = 0x3e84;
      }
      else {
        UVar34 = 0x3e96 - (g_playSystem_00802A38->field_00E4 - g_playSystem_00802A38->field_0034 <
                          6000);
      }
      goto cf_common_exit_00536F8B;
    case CASE_2:
      if (DAT_0080874e == '\x03') {
        pcVar31 = this_00->field_0180;
        pcVar35 = thunk_FUN_0052a280;
        UVar34 = 0x3e8d;
      }
      else {
        pcVar35 = thunk_FUN_0052a1e0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a230;
        }
        pcVar31 = this_00->field_0180;
        UVar34 = 0x3e8d;
      }
      goto cf_common_exit_00536F92;
    case CASE_3:
    case CASE_4:
      UVar34 = 0x2750;
      goto cf_common_exit_00536DB5;
    default:
      goto cf_common_exit_00536FB7;
    case CASE_6:
      if (DAT_0080874e == '\x03') {
        iVar9 = 0;
        pcVar35 = thunk_FUN_0052a770;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3eff;
      }
      else {
        pcVar35 = thunk_FUN_0052a6f0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a730;
        }
        iVar9 = 0;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3eff;
      }
      break;
    case CASE_8:
      if (*(int *)(message->arg0).ptr != 1) {
        if (*(int *)(message->arg0).ptr != 2) {
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        if (DAT_0080874e == '\x03') {
          pcVar35 = thunk_FUN_0052a640;
          UVar34 = 0x3f06;
        }
        else {
          pcVar35 = thunk_FUN_0052a540;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a5c0;
          }
          UVar34 = 0x3f06;
        }
        goto cf_common_exit_00536441;
      }
      if (DAT_0080874e == '\x03') {
        pcVar35 = thunk_FUN_0052a640;
        UVar34 = 0x3ef2;
        goto cf_common_exit_00536441;
      }
LAB_00536d52:
      pcVar35 = thunk_FUN_0052a540;
      if (DAT_0080874e != '\x01') {
        pcVar35 = thunk_FUN_0052a5c0;
      }
      UVar34 = 0x3ef2;
      goto cf_common_exit_00536441;
    case CASE_9:
      if (DAT_0080874e == '\x03') {
        iVar9 = 0;
        pcVar35 = thunk_FUN_0052a770;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3ef7;
      }
      else {
        pcVar35 = thunk_FUN_0052a6f0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a730;
        }
        iVar9 = 0;
        pcVar30 = thunk_FUN_0052a6c0;
        UVar34 = 0x3ef7;
      }
    }
    goto cf_common_exit_00536EE8;
  case MESS_BLDLABPANELTY_C0B3:
    OVar3 = this_00->field_01A4;
    if (OVar3 != CASE_1) {
      if (OVar3 != CASE_2) {
        if (OVar3 != CASE_9) {
          g_currentExceptionFrame = local_8c.previous;
          return 0;
        }
        if (DAT_0080874e == '\x03') {
          iVar9 = 0;
          pcVar35 = thunk_FUN_0052a770;
          pcVar30 = thunk_FUN_0052a6c0;
          UVar34 = 0x3f04;
        }
        else {
          pcVar35 = thunk_FUN_0052a6f0;
          if (DAT_0080874e != '\x01') {
            pcVar35 = thunk_FUN_0052a730;
          }
          iVar9 = 0;
          pcVar30 = thunk_FUN_0052a6c0;
          UVar34 = 0x3f04;
        }
        goto cf_common_exit_00536EE8;
      }
      if (DAT_0080874e == '\x03') {
        pcVar35 = thunk_FUN_0052a280;
        UVar34 = 0x3e8f;
      }
      else {
        pcVar35 = thunk_FUN_0052a1e0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a230;
        }
        UVar34 = 0x3e8f;
      }
      goto cf_common_exit_00536F8B;
    }
    if (DAT_0080874e == '\x03') {
      pcVar31 = this_00->field_0180;
      pcVar35 = thunk_FUN_0052a280;
      UVar34 = 0x3e85;
    }
    else {
      pcVar35 = thunk_FUN_0052a1e0;
      if (DAT_0080874e != '\x01') {
        pcVar35 = thunk_FUN_0052a230;
      }
      pcVar31 = this_00->field_0180;
      UVar34 = 0x3e85;
    }
    goto cf_common_exit_00536F92;
  case MESS_RESEARCHPANELTY_C0B4:
    if (this_00->field_01A4 != CASE_1) {
      if (this_00->field_01A4 != CASE_9) {
        g_currentExceptionFrame = local_8c.previous;
        return 0;
      }
      if (DAT_0080874e == '\x03') {
        pcVar35 = thunk_FUN_0052a770;
      }
      else {
        pcVar35 = thunk_FUN_0052a6f0;
        if (DAT_0080874e != '\x01') {
          pcVar35 = thunk_FUN_0052a730;
        }
      }
      iVar9 = 0;
      pcVar30 = thunk_FUN_0052a6c0;
      UVar34 = 0x3f05;
      goto cf_common_exit_00536EE8;
    }
    if (DAT_0080874e == '\x03') {
      pcVar31 = this_00->field_0180;
      pcVar35 = thunk_FUN_0052a280;
      UVar34 = 0x3e86;
    }
    else {
      pcVar35 = thunk_FUN_0052a1e0;
      if (DAT_0080874e != '\x01') {
        pcVar35 = thunk_FUN_0052a230;
      }
      pcVar31 = this_00->field_0180;
      UVar34 = 0x3e86;
    }
    goto cf_common_exit_00536F92;
  case MESS_SHARED_C0B5:
    if (this_00->field_01A4 != CASE_1) {
      g_currentExceptionFrame = local_8c.previous;
      return 0;
    }
    if (DAT_0080874e == '\x03') {
      pcVar35 = thunk_FUN_0052a280;
    }
    else {
      pcVar35 = thunk_FUN_0052a1e0;
      if (DAT_0080874e != '\x01') {
        pcVar35 = thunk_FUN_0052a230;
      }
    }
    UVar34 = 0x3e87;
cf_common_exit_00536F8B:
    pcVar31 = this_00->field_0180;
cf_common_exit_00536F92:
    uVar33 = 0;
    iVar9 = 1;
    pCVar13 = thunk_FUN_00571240("BUT_BUTOPT",0);
    pCVar13 = FUN_006f2c00(pCVar13,iVar9,uVar33);
    UPanelTy::PaintTxtBut
              ((UPanelTy *)this_00,(AnonShape_0053D920_829E6B3B *)message,1,pCVar13,pcVar31,UVar34,
               pcVar35);
  }
cf_common_exit_00536FB7:
  g_currentExceptionFrame = local_8c.previous;
  return 0;
}

