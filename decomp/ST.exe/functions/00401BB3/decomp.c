
void __fastcall thunk_FUN_006479e0(int param_1)

{
  if (-1 < (int)*(uint *)(param_1 + 0x5d)) {
    FUN_006e8ba0(DAT_00807598,*(uint *)(param_1 + 0x5d));
    *(undefined4 *)(param_1 + 0x5d) = 0xffffffff;
  }
  if (-1 < (int)*(uint *)(param_1 + 0x7a)) {
    FUN_006e8ba0(DAT_00807598,*(uint *)(param_1 + 0x7a));
    *(undefined4 *)(param_1 + 0x7a) = 0xffffffff;
  }
  if (-1 < (int)*(uint *)(param_1 + 0x86)) {
    FUN_006e8ba0(DAT_00807598,*(uint *)(param_1 + 0x86));
    *(undefined4 *)(param_1 + 0x86) = 0xffffffff;
  }
  return;
}

