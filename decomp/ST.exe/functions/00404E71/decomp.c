
void __thiscall
thunk_FUN_00691690(void *this,cMf32 *param_1,undefined4 param_2,int param_3,undefined *param_4)

{
  short *psVar1;
  void *pvVar2;
  undefined4 *puVar3;
  char *pcVar4;
  int *this_00;
  int iVar5;
  undefined *puVar6;
  uint uVar7;
  ushort *puVar8;
  undefined4 *puVar9;
  int iVar10;
  int *piVar11;
  byte bVar12;
  undefined4 uVar13;
  int iVar14;
  undefined4 uVar15;
  CHAR aCStack_190 [260];
  int aiStack_8c [2];
  ushort uStack_84;
  int iStack_70;
  ushort *puStack_6c;
  int iStack_68;
  ushort *puStack_64;
  byte bStack_44;
  uint uStack_3c;
  ushort *puStack_38;
  uint uStack_34;
  ushort *puStack_30;
  uint uStack_2c;
  int *piStack_28;
  char *pcStack_24;
  int iStack_20;
  int *piStack_1c;
  int iStack_18;
  void *pvStack_14;
  int *piStack_10;
  int iStack_c;
  uint uStack_8;
  
  pvStack_14 = this;
  puStack_38 = FUN_006f2d90(param_1,s_2D_CLT_007d59b8,0,0);
  if (puStack_38 != (ushort *)0x0) {
    puVar8 = puStack_38;
    puVar3 = (undefined4 *)((int)this + 0x37ac);
    for (iVar10 = 0xf; iVar10 != 0; iVar10 = iVar10 + -1) {
      *puVar3 = *(undefined4 *)puVar8;
      puVar8 = puVar8 + 2;
      puVar3 = puVar3 + 1;
    }
    cMf32::RecMemFree(param_1,(uint *)&puStack_38);
  }
  puStack_30 = FUN_006f2d90(param_1,s_CLR_COVER_007d59ac,0,0);
  if (puStack_30 != (ushort *)0x0) {
    puVar3 = &DAT_007d5934;
    puVar8 = puStack_30;
    do {
      uVar13 = *(undefined4 *)puVar8;
      puVar8 = puVar8 + 2;
      *puVar3 = uVar13;
      puVar3 = puVar3 + 2;
    } while ((int)puVar3 < 0x7d595c);
    cMf32::RecMemFree(param_1,(uint *)&puStack_30);
  }
  iStack_20 = 0;
  do {
    iVar10 = iStack_20;
    if (param_3 != 0) {
      iVar14 = 1;
      bVar12 = 2;
      pcVar4 = thunk_FUN_00691650(&DAT_007d59a4,iStack_20);
      pcVar4 = FUN_00725c60(param_2,pcVar4,bVar12,iVar14);
      *(char **)((int)this + iVar10 * 0x248 + 0x1950) = pcVar4;
      thunk_FUN_00692ba0((int)pcVar4);
    }
    puVar3 = FUN_006b04d0(0x4f2);
    if (puVar3 == (undefined4 *)0x0) {
      this_00 = (int *)0x0;
LAB_0069177d:
      FUN_006a5e40(-2,DAT_007ed77c,0x7d5980,0x4e);
    }
    else {
      this_00 = (int *)FUN_006dbca0((int)puVar3);
      if (this_00 == (int *)0x0) goto LAB_0069177d;
    }
    iVar10 = iStack_20;
    iVar14 = (int)(0x28 / (longlong)*(int *)(&DAT_007dfa90 + iStack_20 * 4));
    FUN_006dc050(this_00,0,0,0,iVar14,iVar14,0,0xc0240000,0,0x40240000,0,0xc0240000,0,0x40240000,
                 0x4024000000000000,0x4023ee97865e3540,0);
    FUN_006dd610(this_00,4,0,0x40240000,0,0x40240000);
    this_00[0x4a] = 0;
    FUN_006dd790(this_00,149.97069);
    FUN_006dd800(this_00,200.19933);
    iStack_c = 0;
    do {
      uStack_8 = 0;
      do {
        if (((param_3 == 0) || (iStack_c != 0)) || (uStack_8 != 0)) {
          if (param_4 != (undefined *)0x0) {
            (*(code *)param_4)(0);
          }
          uVar7 = uStack_8;
          if (iStack_c == 0) {
            uVar7 = uStack_8 + 1;
          }
          pcVar4 = (char *)thunk_FUN_006a2d00(1,iStack_c,uVar7,1,0);
          puVar8 = FUN_006f0cd0(param_1,pcVar4,0);
          if (puVar8 != (ushort *)0x0) {
            puVar3 = (undefined4 *)FUN_006aac70(0xa9);
            piStack_1c = (int *)(iStack_c * 6);
            piStack_28 = (int *)((int)pvStack_14 +
                                ((int)piStack_1c + uStack_8 + iVar10 * 0x49) * 8 + 0x1954);
            *piStack_28 = (int)puVar3;
            for (iVar14 = 0x2a; iVar14 != 0; iVar14 = iVar14 + -1) {
              *puVar3 = 0;
              puVar3 = puVar3 + 1;
            }
            *(undefined1 *)puVar3 = 0;
            *(undefined2 *)(*piStack_28 + 0x23) = 0;
            iStack_18 = 0;
            pcStack_24 = (char *)0x0;
            *(short *)(*piStack_28 + 0x29) =
                 (short)(0x28 / (longlong)*(int *)(&DAT_007dfa90 + iVar10 * 4));
            *(short *)(*piStack_28 + 0x2b) =
                 (short)(0x28 / (longlong)*(int *)(&DAT_007dfa90 + iVar10 * 4));
            do {
              piStack_10 = (int *)0x0;
              do {
                uVar7 = uStack_8;
                if (iStack_c == 0) {
                  uVar7 = uStack_8 + 1;
                }
                pcVar4 = (char *)thunk_FUN_006a2d00(1,iStack_c,uVar7,iStack_18 + 1,(uint)piStack_10)
                ;
                piVar11 = aiStack_8c;
                for (iVar14 = 0x14; iVar14 != 0; iVar14 = iVar14 + -1) {
                  *piVar11 = 0;
                  piVar11 = piVar11 + 1;
                }
                puVar8 = FUN_006f0cd0(param_1,pcVar4,0);
                if (puVar8 == (ushort *)0x0) {
                  *(short *)(*piStack_28 + 0x23) = *(short *)(*piStack_28 + 0x23) + 1;
                }
                else {
                  if (iStack_c == 0) {
                    iVar14 = (int)piStack_10 * 0x10 + (iStack_18 + uStack_8 * 2) * 0xf0 + 0x2028;
LAB_00691a65:
                    puVar3 = (undefined4 *)(iVar14 + (int)pvStack_14);
                    *puVar3 = *(undefined4 *)(puVar8 + 2);
                    puVar3[1] = *(undefined4 *)(puVar8 + 4);
                    puVar3[2] = *(undefined4 *)(puVar8 + 6);
                    puVar3[3] = *(undefined4 *)(puVar8 + 8);
                  }
                  else if (iStack_c == 1) {
                    iVar14 = (int)piStack_10 * 0x10 + (iStack_18 + uStack_8 * 2) * 0xf0 + 0x2b68;
                    goto LAB_00691a65;
                  }
                  iStack_70 = (int)(short)*puVar8;
                  puStack_6c = puVar8 + 10;
                  iStack_68 = (int)(short)puVar8[1];
                  puStack_64 = puVar8 + (short)*puVar8 * 6 + 10;
                  bStack_44 = (byte)uStack_8;
                  if (iStack_c == 0) {
                    uStack_84 = (ushort)(byte)uStack_8;
                  }
                  else {
                    uStack_84 = ((byte)uStack_8 - 1) + (short)iStack_c;
                  }
                  aiStack_8c[0] = this_00[0x50];
                  this_00[0x4f] = this_00[0x4f] + 1;
                  this_00[0x50] = (int)aiStack_8c;
                  FUN_006ddbe0((int)this_00);
                  this_00[0x49] = 5;
                  FUN_006ddd50(this_00);
                  this_00[0x50] = 0;
                  this_00[0x4f] = 0;
                  if (param_3 != 0) {
                    puVar3 = (undefined4 *)(0x28 / (longlong)*(int *)(&DAT_007dfa90 + iVar10 * 4));
                    uStack_3c = thunk_FUN_00692920(param_2,(undefined4 *)this_00[3],puVar3,
                                                   (int)puVar3,1);
                    piVar11 = piStack_28;
                    FUN_006b4b20((int *)(*piStack_28 + 0x2d +
                                        (int)(pcStack_24 + (int)piStack_10) * 4),uStack_3c,0,0);
                    FUN_006ab060(&uStack_3c);
                    psVar1 = (short *)(*piVar11 + 0x23);
                    *psVar1 = *psVar1 + 1;
                    *(undefined4 *)(*piVar11 + 0x25) = 0;
                  }
                  if (iVar10 == 0) {
                    puVar9 = (undefined4 *)FUN_006aac70(0x640);
                    *(undefined4 **)
                     ((int)pvStack_14 +
                     (int)piStack_10 * 4 + (iStack_18 + ((int)piStack_1c + uStack_8) * 2) * 0x3c) =
                         puVar9;
                    puVar3 = (undefined4 *)this_00[3];
                    for (iVar14 = 400; iVar10 = iStack_20, iVar14 != 0; iVar14 = iVar14 + -1) {
                      *puVar9 = *puVar3;
                      puVar3 = puVar3 + 1;
                      puVar9 = puVar9 + 1;
                    }
                  }
                }
                piStack_10 = (int *)((int)piStack_10 + 1);
              } while ((int)piStack_10 < 0xf);
              iStack_18 = iStack_18 + 1;
              pcStack_24 = pcStack_24 + 0xf;
            } while ((int)pcStack_24 < 0x1e);
          }
          uVar7 = uStack_8;
          if (iStack_c == 0) {
            uVar7 = uStack_8 + 1;
          }
          pcVar4 = (char *)thunk_FUN_006a2d00(0,iStack_c,uVar7,0,1);
          puVar8 = FUN_006f0cd0(param_1,pcVar4,0);
          if (puVar8 != (ushort *)0x0) {
            puVar3 = (undefined4 *)FUN_006aac70(0x59);
            iStack_18 = iStack_c * 6 + iVar10 * 0x92;
            piVar11 = (int *)((int)pvStack_14 + (iStack_18 + uStack_8) * 4 + 0x1a74);
            *piVar11 = (int)puVar3;
            for (iVar14 = 0x16; iVar14 != 0; iVar14 = iVar14 + -1) {
              *puVar3 = 0;
              puVar3 = puVar3 + 1;
            }
            *(undefined1 *)puVar3 = 0;
            piStack_10 = (int *)0x0;
            *(undefined2 *)(*piVar11 + 0x23) = 0;
            *(short *)(*piVar11 + 0x29) =
                 (short)(0x28 / (longlong)*(int *)(&DAT_007dfa90 + iVar10 * 4));
            *(short *)(*piVar11 + 0x2b) =
                 (short)(0x28 / (longlong)*(int *)(&DAT_007dfa90 + iVar10 * 4));
            *(undefined4 *)(*piVar11 + 0x25) = 0;
            do {
              uVar7 = uStack_8;
              if (iStack_c == 0) {
                uVar7 = uStack_8 + 1;
              }
              piStack_1c = (int *)((int)piStack_10 + 1);
              pcVar4 = (char *)thunk_FUN_006a2d00(0,iStack_c,uVar7,0,(uint)piStack_1c);
              piVar11 = aiStack_8c;
              for (iVar14 = 0x14; iVar14 != 0; iVar14 = iVar14 + -1) {
                *piVar11 = 0;
                piVar11 = piVar11 + 1;
              }
              puVar8 = FUN_006f0cd0(param_1,pcVar4,0);
              if (puVar8 == (ushort *)0x0) {
                psVar1 = (short *)(*(int *)((int)pvStack_14 + (iStack_18 + uStack_8) * 4 + 0x1a74) +
                                  0x23);
                *psVar1 = *psVar1 + 1;
              }
              else {
                iStack_70 = (int)(short)*puVar8;
                puStack_6c = puVar8 + 10;
                iStack_68 = (int)(short)puVar8[1];
                puStack_64 = puVar8 + (short)*puVar8 * 6 + 10;
                bStack_44 = (byte)uStack_8;
                if (iStack_c == 0) {
                  uStack_84 = (ushort)(byte)uStack_8;
                }
                else {
                  uStack_84 = ((byte)uStack_8 - 1) + (short)iStack_c;
                }
                aiStack_8c[0] = this_00[0x50];
                this_00[0x4f] = this_00[0x4f] + 1;
                this_00[0x50] = (int)aiStack_8c;
                FUN_006ddbe0((int)this_00);
                this_00[0x49] = 5;
                FUN_006ddd50(this_00);
                pvVar2 = pvStack_14;
                this_00[0x50] = 0;
                this_00[0x4f] = 0;
                if (param_3 != 0) {
                  puVar3 = (undefined4 *)(0x28 / (longlong)*(int *)(&DAT_007dfa90 + iVar10 * 4));
                  uStack_2c = thunk_FUN_00692920(param_2,(undefined4 *)this_00[3],puVar3,(int)puVar3
                                                 ,1);
                  iVar14 = iStack_18 + uStack_8;
                  FUN_006b4b20((int *)(*(int *)((int)pvVar2 + iVar14 * 4 + 0x1a74) + 0x2d +
                                      (int)piStack_10 * 4),uStack_2c,0,0);
                  FUN_006ab060(&uStack_2c);
                  psVar1 = (short *)(*(int *)((int)pvVar2 + iVar14 * 4 + 0x1a74) + 0x23);
                  *psVar1 = *psVar1 + 1;
                }
                if (iVar10 == 0) {
                  puVar9 = (undefined4 *)FUN_006aac70(0x640);
                  *(undefined4 **)
                   ((int)pvStack_14 +
                   ((int)piStack_10 + (uStack_8 + (iStack_c * 3 + 0x36) * 2) * 10) * 4) = puVar9;
                  puVar3 = (undefined4 *)this_00[3];
                  for (iVar14 = 400; iVar10 = iStack_20, iVar14 != 0; iVar14 = iVar14 + -1) {
                    *puVar9 = *puVar3;
                    puVar3 = puVar3 + 1;
                    puVar9 = puVar9 + 1;
                  }
                }
              }
              piStack_10 = piStack_1c;
            } while ((int)piStack_1c < 10);
          }
          uVar7 = uStack_8;
          if (iStack_c == 0) {
            uVar7 = uStack_8 + 1;
          }
          pcVar4 = (char *)thunk_FUN_006a2d00(3,iStack_c,uVar7,1,1);
          puVar8 = FUN_006f0cd0(param_1,pcVar4,0);
          if (puVar8 != (ushort *)0x0) {
            puVar3 = (undefined4 *)FUN_006aac70(0x45);
            iStack_18 = iStack_c * 6 + iVar10 * 0x92;
            piStack_1c = (int *)((int)pvStack_14 + (iStack_18 + uStack_8) * 4 + 0x1b08);
            *piStack_1c = (int)puVar3;
            for (iVar14 = 0x11; iVar14 != 0; iVar14 = iVar14 + -1) {
              *puVar3 = 0;
              puVar3 = puVar3 + 1;
            }
            *(undefined1 *)puVar3 = 0;
            piStack_10 = (int *)0x0;
            *(undefined2 *)(*piStack_1c + 0x23) = 0;
            *(short *)(*piStack_1c + 0x29) =
                 (short)(0x28 / (longlong)*(int *)(&DAT_007dfa90 + iVar10 * 4));
            *(short *)(*piStack_1c + 0x2b) =
                 (short)(0x28 / (longlong)*(int *)(&DAT_007dfa90 + iVar10 * 4));
            *(undefined4 *)(*piStack_1c + 0x25) = 0;
            do {
              uVar7 = uStack_8;
              if (iStack_c == 0) {
                uVar7 = uStack_8 + 1;
              }
              pcVar4 = (char *)thunk_FUN_006a2d00(3,iStack_c,uVar7,1,(uint)piStack_10);
              puVar8 = FUN_006f0cd0(param_1,pcVar4,0);
              piVar11 = aiStack_8c;
              for (iVar14 = 0x14; iVar14 != 0; iVar14 = iVar14 + -1) {
                *piVar11 = 0;
                piVar11 = piVar11 + 1;
              }
              if (puVar8 == (ushort *)0x0) {
                psVar1 = (short *)(*(int *)((int)pvStack_14 + (iStack_18 + uStack_8) * 4 + 0x1b08) +
                                  0x23);
                *psVar1 = *psVar1 + 1;
              }
              else {
                iStack_70 = (int)(short)*puVar8;
                puStack_6c = puVar8 + 10;
                iStack_68 = (int)(short)puVar8[1];
                bStack_44 = (byte)uStack_8;
                puStack_64 = puVar8 + (short)*puVar8 * 6 + 10;
                if (iStack_c == 0) {
                  uStack_84 = (ushort)(byte)uStack_8;
                }
                else {
                  uStack_84 = ((byte)uStack_8 - 1) + (short)iStack_c;
                }
                aiStack_8c[0] = this_00[0x50];
                this_00[0x4f] = this_00[0x4f] + 1;
                this_00[0x50] = (int)aiStack_8c;
                FUN_006ddbe0((int)this_00);
                this_00[0x49] = 5;
                FUN_006ddd50(this_00);
                pvVar2 = pvStack_14;
                this_00[0x50] = 0;
                this_00[0x4f] = 0;
                if (param_3 != 0) {
                  puVar3 = (undefined4 *)(0x28 / (longlong)*(int *)(&DAT_007dfa90 + iVar10 * 4));
                  uStack_34 = thunk_FUN_00692920(param_2,(undefined4 *)this_00[3],puVar3,(int)puVar3
                                                 ,1);
                  iVar14 = iStack_18 + uStack_8;
                  FUN_006b4b20((int *)(*(int *)((int)pvVar2 + iVar14 * 4 + 0x1b08) + 0x2d +
                                      (int)piStack_10 * 4),uStack_34,0,0);
                  FUN_006ab060(&uStack_34);
                  psVar1 = (short *)(*(int *)((int)pvVar2 + iVar14 * 4 + 0x1b08) + 0x23);
                  *psVar1 = *psVar1 + 1;
                }
                if (iVar10 == 0) {
                  puVar9 = (undefined4 *)FUN_006aac70(0x640);
                  *(undefined4 **)
                   ((int)pvStack_14 +
                   ((int)piStack_10 + (uStack_8 + (iStack_c * 3 + 0x90) * 2) * 5) * 4) = puVar9;
                  puVar3 = (undefined4 *)this_00[3];
                  for (iVar14 = 400; iVar10 = iStack_20, iVar14 != 0; iVar14 = iVar14 + -1) {
                    *puVar9 = *puVar3;
                    puVar3 = puVar3 + 1;
                    puVar9 = puVar9 + 1;
                  }
                }
              }
              piStack_10 = (int *)((int)piStack_10 + 1);
            } while ((int)piStack_10 < 5);
          }
        }
        else {
          iVar14 = 0;
          do {
            pcStack_24 = thunk_FUN_00692390(0,0,iVar14,iVar10);
            iVar5 = FUN_006f21c0(7,pcStack_24);
            if (iVar5 != 0) break;
            pcVar4 = FUN_00725c60(param_2,pcStack_24,2,1);
            *(char **)((int)pvStack_14 + (iVar14 + iVar10 * 0x92) * 4 + 0x1954) = pcVar4;
            thunk_FUN_00692ba0((int)pcVar4);
            iVar14 = iVar14 + 1;
          } while (iVar14 < 2);
          pvVar2 = pvStack_14;
          uVar15 = 0;
          uVar13 = 0;
          puVar6 = thunk_FUN_00691650(s_STONE_007d5978,iVar10);
          wsprintfA(aCStack_190,s__s_u_u_007d5970,puVar6,uVar13,uVar15);
          iVar14 = FUN_006f21c0(7,aCStack_190);
          if (iVar14 == 0) {
            pcVar4 = FUN_00725c60(param_2,aCStack_190,2,1);
            *(char **)((int)pvVar2 + iVar10 * 0x248 + 0x1a74) = pcVar4;
            thunk_FUN_00692ba0((int)pcVar4);
          }
        }
        pvVar2 = pvStack_14;
        uStack_8 = uStack_8 + 1;
      } while ((int)uStack_8 < 6);
      iStack_c = iStack_c + 1;
    } while (iStack_c < 6);
    if (param_3 != 0) {
      iVar14 = 1;
      bVar12 = 2;
      pcVar4 = thunk_FUN_00691650(s_CRACK_007d5968,iVar10);
      pcVar4 = FUN_00725c60(param_2,pcVar4,bVar12,iVar14);
      *(char **)((int)pvVar2 + iVar10 * 0x248 + 0x1b04) = pcVar4;
      thunk_FUN_00692ba0((int)pcVar4);
    }
    if (this_00 != (int *)0x0) {
      FUN_006dbcf0(this_00);
      FUN_0072e2b0(this_00);
    }
    iStack_20 = iVar10 + 1;
    this = pvStack_14;
    if (2 < iStack_20) {
      return;
    }
  } while( true );
}

