
/* Recovered from embedded debug metadata:
   E:\__titans\wlad\To_boat.cpp
   STBoatC::ReadyForLoading */

void __thiscall STBoatC::ReadyForLoading(STBoatC *this,void *param_1)

{
  code *pcVar1;
  int iVar2;
  
  if ((((this->field_045D == 0xf) || (this->field_045D == 0xe)) &&
      ((void *)this->field_058E == param_1)) && (this->field_0596 == 2)) {
    FUN_006e62d0(DAT_00802a38,(int)this->field_058E,(int *)&param_1);
    thunk_FUN_004162f0(param_1,(undefined2 *)&this->field_0x588,&this->field_058A,&this->field_058C)
    ;
    this->field_0596 = 3;
    this->field_059A = 0;
    return;
  }
  iVar2 = ReportDebugMessage(s_E____titans_wlad_To_boat_cpp_007a9d3c,0x50bb,0,0,&DAT_007a4ccc,
                             s_STBoatC__ReadyForLoading_incorre_007abb28);
  if (iVar2 != 0) {
    pcVar1 = (code *)swi(3);
    (*pcVar1)();
    return;
  }
  return;
}

