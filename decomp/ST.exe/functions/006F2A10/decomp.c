
/* Recovered from embedded debug metadata:
   E:\Ourlib\Mf32int.cpp
   cMf32::RecKeyGetPrev */

undefined4 * __thiscall cMf32::RecKeyGetPrev(cMf32 *this,undefined4 *param_1)

{
  code *pcVar1;
  int iVar2;
  DWORD DVar3;
  int iVar4;
  undefined4 *puVar5;
  undefined4 unaff_ESI;
  void *unaff_EDI;
  InternalExceptionFrame local_4c;
  cMf32 *local_8;
  
  local_4c.previous = g_currentExceptionFrame;
  g_currentExceptionFrame = &local_4c;
  local_8 = this;
  iVar2 = Library::MSVCRT::__setjmp3(local_4c.jumpBuffer,0,unaff_EDI,unaff_ESI);
  if (iVar2 != 0) {
    g_currentExceptionFrame = local_4c.previous;
    iVar4 = ReportDebugMessage(s_E__Ourlib_Mf32int_cpp_007efaa4,0x35a,0,iVar2,&DAT_007a4ccc,
                               s_cMf32__RecKeyGetPrev_007efd10);
    if (iVar4 != 0) {
      pcVar1 = (code *)swi(3);
      puVar5 = (undefined4 *)(*pcVar1)();
      return puVar5;
    }
    RaiseInternalException(iVar2,0,s_E__Ourlib_Mf32int_cpp_007efaa4,0x35c);
    return (undefined4 *)0x0;
  }
  if (param_1 == (undefined4 *)0x0) {
    param_1 = (undefined4 *)&local_8->field_0018;
  }
  puVar5 = param_1;
  for (iVar2 = 0x86; iVar2 != 0; iVar2 = iVar2 + -1) {
    *puVar5 = 0;
    puVar5 = puVar5 + 1;
  }
  *(undefined1 *)puVar5 = 0;
  DVar3 = FUN_00751bf0((int *)local_8->field_0000,param_1,(undefined4 *)0x0,0);
  if (DVar3 == 0xfffffffc) {
    g_currentExceptionFrame = local_4c.previous;
    return (undefined4 *)0x0;
  }
  g_currentExceptionFrame = local_4c.previous;
  *(undefined1 *)(*(short *)((int)param_1 + 0x16) + 0x18 + (int)param_1) = 0;
  return param_1;
}

