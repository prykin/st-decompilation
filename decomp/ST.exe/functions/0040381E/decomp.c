
void __fastcall thunk_FUN_00420340(int *param_1)

{
  void *this;
  char cVar1;
  code *pcVar2;
  byte bVar3;
  undefined4 uVar4;
  uint uVar5;
  int iVar6;
  uint uVar7;
  int iVar8;
  byte *pbVar9;
  char *pcVar10;
  char *pcVar11;
  byte *pbVar12;
  byte abStack_24 [20];
  char acStack_10 [12];
  
  uVar5 = 0xffffffff;
  pcVar10 = &DAT_007a5000;
  do {
    pcVar11 = pcVar10;
    if (uVar5 == 0) break;
    uVar5 = uVar5 - 1;
    pcVar11 = pcVar10 + 1;
    cVar1 = *pcVar10;
    pcVar10 = pcVar11;
  } while (cVar1 != '\0');
  uVar5 = ~uVar5;
  abStack_24[0] = 0;
  iVar6 = -1;
  pbVar9 = abStack_24;
  do {
    pbVar12 = pbVar9;
    if (iVar6 == 0) break;
    iVar6 = iVar6 + -1;
    pbVar12 = pbVar9 + 1;
    bVar3 = *pbVar9;
    pbVar9 = pbVar12;
  } while (bVar3 != 0);
  pbVar9 = (byte *)(pcVar11 + -uVar5);
  pbVar12 = pbVar12 + -1;
  for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
    *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
    pbVar9 = pbVar9 + 4;
    pbVar12 = pbVar12 + 4;
  }
  iVar6 = 10;
  for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
    *pbVar12 = *pbVar9;
    pbVar9 = pbVar9 + 1;
    pbVar12 = pbVar12 + 1;
  }
  uVar5 = 0xffffffff;
  pcVar10 = &DAT_007a4ffc;
  do {
    pcVar11 = pcVar10;
    if (uVar5 == 0) break;
    uVar5 = uVar5 - 1;
    pcVar11 = pcVar10 + 1;
    cVar1 = *pcVar10;
    pcVar10 = pcVar11;
  } while (cVar1 != '\0');
  uVar5 = ~uVar5;
  iVar8 = -1;
  pbVar9 = abStack_24;
  do {
    pbVar12 = pbVar9;
    if (iVar8 == 0) break;
    iVar8 = iVar8 + -1;
    pbVar12 = pbVar9 + 1;
    bVar3 = *pbVar9;
    pbVar9 = pbVar12;
  } while (bVar3 != 0);
  pbVar9 = (byte *)(pcVar11 + -uVar5);
  pbVar12 = pbVar12 + -1;
  for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
    *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
    pbVar9 = pbVar9 + 4;
    pbVar12 = pbVar12 + 4;
  }
  pcVar10 = acStack_10;
  for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
    *pbVar12 = *pbVar9;
    pbVar9 = pbVar9 + 1;
    pbVar12 = pbVar12 + 1;
  }
  bVar3 = thunk_FUN_004ab030();
  __itoa((uint)bVar3,pcVar10,iVar6);
  iVar6 = param_1[8];
  if ((((iVar6 != 0x1ae) && (iVar6 != 0x172)) && (iVar6 != 0x5a)) &&
     ((iVar6 != 0x1a4 && (iVar6 != 0x1b8)))) {
    if ((iVar6 == 0x14) || (iVar6 == 1000)) {
      uVar4 = (**(code **)(*param_1 + 0x2c))();
    }
    else {
      if (iVar6 != 0x3e9) {
        return;
      }
      uVar4 = *(undefined4 *)((int)param_1 + 0x259);
    }
    switch(uVar4) {
    case 1:
    case 9:
    case 0x15:
    case 0x24:
    case 0x25:
switchD_00420436_caseD_1:
      uVar5 = 0xffffffff;
      pcVar10 = &DAT_007a4fb4;
      do {
        pcVar11 = pcVar10;
        if (uVar5 == 0) break;
        uVar5 = uVar5 - 1;
        pcVar11 = pcVar10 + 1;
        cVar1 = *pcVar10;
        pcVar10 = pcVar11;
      } while (cVar1 != '\0');
      uVar5 = ~uVar5;
      iVar6 = -1;
      pbVar9 = abStack_24;
      do {
        pbVar12 = pbVar9;
        if (iVar6 == 0) break;
        iVar6 = iVar6 + -1;
        pbVar12 = pbVar9 + 1;
        bVar3 = *pbVar9;
        pbVar9 = pbVar12;
      } while (bVar3 != 0);
      pbVar9 = (byte *)(pcVar11 + -uVar5);
      pbVar12 = pbVar12 + -1;
      for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
        *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
        pbVar9 = pbVar9 + 4;
        pbVar12 = pbVar12 + 4;
      }
      for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
        *pbVar12 = *pbVar9;
        pbVar9 = pbVar9 + 1;
        pbVar12 = pbVar12 + 1;
      }
      uVar5 = 0xffffffff;
      pcVar10 = acStack_10;
      do {
        pcVar11 = pcVar10;
        if (uVar5 == 0) break;
        uVar5 = uVar5 - 1;
        pcVar11 = pcVar10 + 1;
        cVar1 = *pcVar10;
        pcVar10 = pcVar11;
      } while (cVar1 != '\0');
      uVar5 = ~uVar5;
      iVar6 = -1;
      pbVar9 = abStack_24;
      do {
        pbVar12 = pbVar9;
        if (iVar6 == 0) break;
        iVar6 = iVar6 + -1;
        pbVar12 = pbVar9 + 1;
        bVar3 = *pbVar9;
        pbVar9 = pbVar12;
      } while (bVar3 != 0);
      pbVar9 = (byte *)(pcVar11 + -uVar5);
      pbVar12 = pbVar12 + -1;
      for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
        *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
        pbVar9 = pbVar9 + 4;
        pbVar12 = pbVar12 + 4;
      }
      for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
        *pbVar12 = *pbVar9;
        pbVar9 = pbVar9 + 1;
        pbVar12 = pbVar12 + 1;
      }
      pcVar10 = &DAT_007a4fb0;
      break;
    case 2:
    case 3:
    case 4:
    case 5:
    case 6:
    case 8:
    case 10:
    case 0xb:
    case 0xc:
    case 0xd:
    case 0xe:
    case 0xf:
    case 0x10:
    case 0x11:
    case 0x12:
    case 0x14:
    case 0x16:
    case 0x17:
    case 0x18:
    case 0x19:
    case 0x1a:
    case 0x1b:
    case 0x1c:
    case 0x1d:
    case 0x1e:
    case 0x1f:
    case 0x20:
    case 0x21:
    case 0x22:
    case 0x23:
    case 0x26:
    case 0x27:
    case 0x28:
      goto switchD_00420436_caseD_2;
    case 7:
    case 0x13:
    case 0x36:
    case 0x38:
    case 0x39:
    case 0x3a:
    case 0x3e:
    case 0x3f:
    case 0x40:
    case 0x42:
    case 0x44:
    case 0x45:
    case 0x46:
    case 0x47:
    case 0x49:
    case 0x4a:
    case 0x4b:
    case 0x4c:
    case 0x4e:
    case 0x4f:
    case 0x51:
    case 0x54:
    case 0x55:
    case 0x56:
    case 0x57:
    case 0x58:
    case 0x59:
    case 0x5a:
    case 0x5e:
    case 0x61:
    case 0x65:
    case 0x66:
    case 0x67:
    case 0x68:
    case 0x69:
    case 0x6a:
    case 0x6b:
    case 0x6f:
    case 0x71:
    case 0x72:
      goto switchD_00420436_caseD_7;
    default:
      iVar6 = FUN_006ad4d0(s_E____titans_wlad_Tc_gobj_cpp_007a4e0c,0xc45,0,0,&DAT_007a4ccc);
      if (iVar6 == 0) {
        return;
      }
      pcVar2 = (code *)swi(3);
      (*pcVar2)();
      return;
    case 0x32:
    case 0x33:
    case 0x34:
    case 0x35:
    case 0x37:
    case 0x4d:
    case 0x52:
    case 0x53:
    case 0x5b:
    case 0x5c:
    case 0x5f:
    case 0x60:
    case 100:
    case 0x6e:
      uVar5 = 0xffffffff;
      pcVar10 = &DAT_007a4ff8;
      do {
        pcVar11 = pcVar10;
        if (uVar5 == 0) break;
        uVar5 = uVar5 - 1;
        pcVar11 = pcVar10 + 1;
        cVar1 = *pcVar10;
        pcVar10 = pcVar11;
      } while (cVar1 != '\0');
      uVar5 = ~uVar5;
      iVar6 = -1;
      pbVar9 = abStack_24;
      do {
        pbVar12 = pbVar9;
        if (iVar6 == 0) break;
        iVar6 = iVar6 + -1;
        pbVar12 = pbVar9 + 1;
        bVar3 = *pbVar9;
        pbVar9 = pbVar12;
      } while (bVar3 != 0);
      pbVar9 = (byte *)(pcVar11 + -uVar5);
      pbVar12 = pbVar12 + -1;
      for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
        *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
        pbVar9 = pbVar9 + 4;
        pbVar12 = pbVar12 + 4;
      }
      for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
        *pbVar12 = *pbVar9;
        pbVar9 = pbVar9 + 1;
        pbVar12 = pbVar12 + 1;
      }
      uVar5 = 0xffffffff;
      pcVar10 = acStack_10;
      do {
        pcVar11 = pcVar10;
        if (uVar5 == 0) break;
        uVar5 = uVar5 - 1;
        pcVar11 = pcVar10 + 1;
        cVar1 = *pcVar10;
        pcVar10 = pcVar11;
      } while (cVar1 != '\0');
      uVar5 = ~uVar5;
      iVar6 = -1;
      pbVar9 = abStack_24;
      do {
        pbVar12 = pbVar9;
        if (iVar6 == 0) break;
        iVar6 = iVar6 + -1;
        pbVar12 = pbVar9 + 1;
        bVar3 = *pbVar9;
        pbVar9 = pbVar12;
      } while (bVar3 != 0);
      pbVar9 = (byte *)(pcVar11 + -uVar5);
      pbVar12 = pbVar12 + -1;
      for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
        *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
        pbVar9 = pbVar9 + 4;
        pbVar12 = pbVar12 + 4;
      }
      for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
        *pbVar12 = *pbVar9;
        pbVar9 = pbVar9 + 1;
        pbVar12 = pbVar12 + 1;
      }
      pcVar10 = &DAT_007a4ff8;
      break;
    case 0x3b:
    case 0x3c:
    case 0x3d:
    case 0x41:
    case 0x43:
    case 0x48:
    case 0x50:
    case 0x5d:
    case 0x62:
    case 99:
    case 0x6c:
    case 0x6d:
    case 0x70:
    case 0x73:
      uVar5 = 0xffffffff;
      pcVar10 = &DAT_007a4ff8;
      do {
        pcVar11 = pcVar10;
        if (uVar5 == 0) break;
        uVar5 = uVar5 - 1;
        pcVar11 = pcVar10 + 1;
        cVar1 = *pcVar10;
        pcVar10 = pcVar11;
      } while (cVar1 != '\0');
      uVar5 = ~uVar5;
      iVar6 = -1;
      pbVar9 = abStack_24;
      do {
        pbVar12 = pbVar9;
        if (iVar6 == 0) break;
        iVar6 = iVar6 + -1;
        pbVar12 = pbVar9 + 1;
        bVar3 = *pbVar9;
        pbVar9 = pbVar12;
      } while (bVar3 != 0);
      pbVar9 = (byte *)(pcVar11 + -uVar5);
      pbVar12 = pbVar12 + -1;
      for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
        *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
        pbVar9 = pbVar9 + 4;
        pbVar12 = pbVar12 + 4;
      }
      for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
        *pbVar12 = *pbVar9;
        pbVar9 = pbVar9 + 1;
        pbVar12 = pbVar12 + 1;
      }
      uVar5 = 0xffffffff;
      pcVar10 = acStack_10;
      do {
        pcVar11 = pcVar10;
        if (uVar5 == 0) break;
        uVar5 = uVar5 - 1;
        pcVar11 = pcVar10 + 1;
        cVar1 = *pcVar10;
        pcVar10 = pcVar11;
      } while (cVar1 != '\0');
      uVar5 = ~uVar5;
      iVar6 = -1;
      pbVar9 = abStack_24;
      do {
        pbVar12 = pbVar9;
        if (iVar6 == 0) break;
        iVar6 = iVar6 + -1;
        pbVar12 = pbVar9 + 1;
        bVar3 = *pbVar9;
        pbVar9 = pbVar12;
      } while (bVar3 != 0);
      pbVar9 = (byte *)(pcVar11 + -uVar5);
      pbVar12 = pbVar12 + -1;
      for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
        *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
        pbVar9 = pbVar9 + 4;
        pbVar12 = pbVar12 + 4;
      }
      for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
        *pbVar12 = *pbVar9;
        pbVar9 = pbVar9 + 1;
        pbVar12 = pbVar12 + 1;
      }
      pcVar10 = &DAT_007a4ff4;
    }
LAB_00420678:
    uVar5 = 0xffffffff;
    do {
      pcVar11 = pcVar10;
      if (uVar5 == 0) break;
      uVar5 = uVar5 - 1;
      pcVar11 = pcVar10 + 1;
      cVar1 = *pcVar10;
      pcVar10 = pcVar11;
    } while (cVar1 != '\0');
    uVar5 = ~uVar5;
    iVar6 = -1;
    pbVar9 = abStack_24;
    do {
      pbVar12 = pbVar9;
      if (iVar6 == 0) break;
      iVar6 = iVar6 + -1;
      pbVar12 = pbVar9 + 1;
      bVar3 = *pbVar9;
      pbVar9 = pbVar12;
    } while (bVar3 != 0);
    pbVar9 = (byte *)(pcVar11 + -uVar5);
    pbVar12 = pbVar12 + -1;
    for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
      *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
      pbVar9 = pbVar9 + 4;
      pbVar12 = pbVar12 + 4;
    }
    for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
      *pbVar12 = *pbVar9;
      pbVar9 = pbVar9 + 1;
      pbVar12 = pbVar12 + 1;
    }
    this = (void *)((int)param_1 + 0x1d5);
    iVar6 = thunk_FUN_004ab880(0xf,DAT_00806774,abStack_24,0x1d);
    if (iVar6 != 0) {
      FUN_006a5e40(-0x5001fff8,DAT_007ed77c,0x7a4e0c,0xc4a);
    }
    iVar6 = thunk_FUN_004ab880(0,DAT_00806774,abStack_24,0x1d);
    if (iVar6 != 0) {
      FUN_006a5e40(-0x5001fff8,DAT_007ed77c,0x7a4e0c,0xc4b);
    }
    FUN_006ea5e0(*(void **)((int)param_1 + 0x211),*(uint *)((int)param_1 + 0x1ed),0xf,0);
    FUN_006ea5e0(*(void **)((int)param_1 + 0x211),*(uint *)((int)param_1 + 0x1ed),0,0);
    if (DAT_0080732c != 1) {
      thunk_FUN_004ac410(0xf);
      thunk_FUN_004ac410(0);
      thunk_FUN_004abce0(this,0,0,0,'\0');
      thunk_FUN_004abce0(this,0xf,0x14,0x14,'\0');
      thunk_FUN_004ac1a0(0,*(undefined4 *)(DAT_00802a38 + 0xe4));
      thunk_FUN_004ac1a0(0xf,*(undefined4 *)(DAT_00802a38 + 0xe4));
      return;
    }
    thunk_FUN_004abce0(this,0,0,0x13,'\0');
    thunk_FUN_004abce0(this,0xf,0x14,0x27,'\0');
    thunk_FUN_004acd60(this,'\0',*(uint *)(DAT_00802a38 + 0xe4) & 1);
    thunk_FUN_004acd60(this,'\x0f',*(uint *)(DAT_00802a38 + 0xe4) & 1);
    thunk_FUN_004ac1a0(0,*(undefined4 *)(DAT_00802a38 + 0xe4));
    thunk_FUN_004ac1a0(0xf,*(undefined4 *)(DAT_00802a38 + 0xe4));
    return;
  }
  uVar4 = (**(code **)(*param_1 + 0x2c))();
  switch(uVar4) {
  case 0xa6:
  case 0xa7:
  case 0xaf:
  case 0xbd:
    goto switchD_00420436_caseD_1;
  default:
    iVar6 = FUN_006ad4d0(s_E____titans_wlad_Tc_gobj_cpp_007a4e0c,0xbc0,0,0,&DAT_007a4ccc);
    if (iVar6 == 0) {
      return;
    }
    pcVar2 = (code *)swi(3);
    (*pcVar2)();
    return;
  case 0xdd:
  case 0xde:
  case 0xe0:
switchD_00420436_caseD_7:
    uVar5 = 0xffffffff;
    pcVar10 = &DAT_007a4fb4;
    do {
      pcVar11 = pcVar10;
      if (uVar5 == 0) break;
      uVar5 = uVar5 - 1;
      pcVar11 = pcVar10 + 1;
      cVar1 = *pcVar10;
      pcVar10 = pcVar11;
    } while (cVar1 != '\0');
    uVar5 = ~uVar5;
    iVar6 = -1;
    pbVar9 = abStack_24;
    do {
      pbVar12 = pbVar9;
      if (iVar6 == 0) break;
      iVar6 = iVar6 + -1;
      pbVar12 = pbVar9 + 1;
      bVar3 = *pbVar9;
      pbVar9 = pbVar12;
    } while (bVar3 != 0);
    pbVar9 = (byte *)(pcVar11 + -uVar5);
    pbVar12 = pbVar12 + -1;
    for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
      *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
      pbVar9 = pbVar9 + 4;
      pbVar12 = pbVar12 + 4;
    }
    for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
      *pbVar12 = *pbVar9;
      pbVar9 = pbVar9 + 1;
      pbVar12 = pbVar12 + 1;
    }
    uVar5 = 0xffffffff;
    pcVar10 = acStack_10;
    do {
      pcVar11 = pcVar10;
      if (uVar5 == 0) break;
      uVar5 = uVar5 - 1;
      pcVar11 = pcVar10 + 1;
      cVar1 = *pcVar10;
      pcVar10 = pcVar11;
    } while (cVar1 != '\0');
    uVar5 = ~uVar5;
    iVar6 = -1;
    pbVar9 = abStack_24;
    do {
      pbVar12 = pbVar9;
      if (iVar6 == 0) break;
      iVar6 = iVar6 + -1;
      pbVar12 = pbVar9 + 1;
      bVar3 = *pbVar9;
      pbVar9 = pbVar12;
    } while (bVar3 != 0);
    pbVar9 = (byte *)(pcVar11 + -uVar5);
    pbVar12 = pbVar12 + -1;
    for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
      *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
      pbVar9 = pbVar9 + 4;
      pbVar12 = pbVar12 + 4;
    }
    for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
      *pbVar12 = *pbVar9;
      pbVar9 = pbVar9 + 1;
      pbVar12 = pbVar12 + 1;
    }
    pcVar10 = &DAT_007a4fac;
    goto LAB_00420678;
  case 0xfd:
  case 0xfe:
switchD_00420436_caseD_2:
    uVar5 = 0xffffffff;
    pcVar10 = &DAT_007a4fb4;
    do {
      pcVar11 = pcVar10;
      if (uVar5 == 0) break;
      uVar5 = uVar5 - 1;
      pcVar11 = pcVar10 + 1;
      cVar1 = *pcVar10;
      pcVar10 = pcVar11;
    } while (cVar1 != '\0');
    uVar5 = ~uVar5;
    iVar6 = -1;
    pbVar9 = abStack_24;
    do {
      pbVar12 = pbVar9;
      if (iVar6 == 0) break;
      iVar6 = iVar6 + -1;
      pbVar12 = pbVar9 + 1;
      bVar3 = *pbVar9;
      pbVar9 = pbVar12;
    } while (bVar3 != 0);
    pbVar9 = (byte *)(pcVar11 + -uVar5);
    pbVar12 = pbVar12 + -1;
    for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
      *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
      pbVar9 = pbVar9 + 4;
      pbVar12 = pbVar12 + 4;
    }
    for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
      *pbVar12 = *pbVar9;
      pbVar9 = pbVar9 + 1;
      pbVar12 = pbVar12 + 1;
    }
    uVar5 = 0xffffffff;
    pcVar10 = acStack_10;
    do {
      pcVar11 = pcVar10;
      if (uVar5 == 0) break;
      uVar5 = uVar5 - 1;
      pcVar11 = pcVar10 + 1;
      cVar1 = *pcVar10;
      pcVar10 = pcVar11;
    } while (cVar1 != '\0');
    uVar5 = ~uVar5;
    iVar6 = -1;
    pbVar9 = abStack_24;
    do {
      pbVar12 = pbVar9;
      if (iVar6 == 0) break;
      iVar6 = iVar6 + -1;
      pbVar12 = pbVar9 + 1;
      bVar3 = *pbVar9;
      pbVar9 = pbVar12;
    } while (bVar3 != 0);
    pbVar9 = (byte *)(pcVar11 + -uVar5);
    pbVar12 = pbVar12 + -1;
    for (uVar7 = uVar5 >> 2; uVar7 != 0; uVar7 = uVar7 - 1) {
      *(undefined4 *)pbVar12 = *(undefined4 *)pbVar9;
      pbVar9 = pbVar9 + 4;
      pbVar12 = pbVar12 + 4;
    }
    for (uVar5 = uVar5 & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
      *pbVar12 = *pbVar9;
      pbVar9 = pbVar9 + 1;
      pbVar12 = pbVar12 + 1;
    }
    pcVar10 = &DAT_007a4fb4;
    goto LAB_00420678;
  }
}

