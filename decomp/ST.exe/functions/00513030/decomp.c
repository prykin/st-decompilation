
/* Recovered from embedded debug metadata:
   E:\__titans\Andrey\helppan.cpp
   HelpPanelTy::ChangeTree */

void __thiscall HelpPanelTy::ChangeTree(HelpPanelTy *this,int *param_1,int param_2)

{
  HelpPanelTy *pHVar1;
  undefined4 *puVar2;
  int iVar3;
  undefined4 uVar4;
  code *pcVar5;
  bool bVar6;
  HelpPanelTy *this_00;
  byte bVar7;
  int iVar8;
  int iVar9;
  uint uVar10;
  int *piVar11;
  uint uVar12;
  int *piVar13;
  undefined4 unaff_ESI;
  void *unaff_EDI;
  int iVar14;
  HelpPanelTy *pHVar15;
  int local_d0 [20];
  undefined4 local_80;
  undefined4 local_7c [16];
  undefined4 local_3c;
  undefined4 local_38;
  undefined4 local_34;
  undefined4 local_30;
  undefined1 local_2c;
  char local_2b;
  undefined1 local_2a;
  undefined1 local_29;
  uint local_28;
  uint local_24;
  HelpPanelTy *local_20;
  uint local_1c;
  uint local_18;
  int *local_14;
  uint local_10;
  uint local_c;
  undefined1 local_6;
  char local_5;
  
  local_18 = param_2 + 1;
  local_14 = param_1;
  local_1c = 0;
  local_80 = DAT_00858df8;
  DAT_00858df8 = &local_80;
  local_20 = this;
  iVar8 = __setjmp3(local_7c,0,unaff_EDI,unaff_ESI);
  piVar13 = local_14;
  this_00 = local_20;
  if (iVar8 != 0) {
    DAT_00858df8 = (undefined4 *)local_80;
    iVar9 = FUN_006ad4d0(s_E____titans_Andrey_helppan_cpp_007c383c,0x26d,0,iVar8,&DAT_007a4ccc);
    if (iVar9 != 0) {
      pcVar5 = (code *)swi(3);
      (*pcVar5)();
      return;
    }
    FUN_006a5e40(iVar8,0,0x7c383c,0x26d);
    return;
  }
  if (*(char *)((int)local_14 + 0x12) == '\0') {
    uVar10 = 0;
    piVar11 = local_d0;
    for (iVar8 = 0x14; iVar8 != 0; iVar8 = iVar8 + -1) {
      *piVar11 = 0;
      piVar11 = piVar11 + 1;
    }
    local_10 = local_10 & 0xffffff00;
    local_5 = '\0';
    if (*(uint *)sizeHelp_exref != 0) {
      piVar11 = *(int **)(this_00 + 0x1c7);
      do {
        if ((*piVar11 == *piVar13) && (piVar11[1] == piVar13[1])) {
          local_1c = uVar10;
          break;
        }
        uVar10 = uVar10 + 1;
        piVar11 = (int *)((int)piVar11 + 0x11);
      } while (uVar10 < *(uint *)sizeHelp_exref);
    }
    iVar9 = piVar13[1];
    local_24 = local_1c & 0xffff;
    iVar8 = local_24 - 1;
    local_c = local_24;
    if (0 < iVar8) {
      iVar14 = iVar8 * 0x11;
      do {
        if (iVar9 == 0) break;
        iVar3 = *(int *)(this_00 + 0x1c7);
        if (*(int *)(iVar3 + iVar14) == iVar9) {
          local_d0[local_10 & 0xff] = iVar9;
          iVar9 = ((int *)(iVar3 + iVar14))[1];
          local_10 = CONCAT31(local_10._1_3_,(byte)local_10 + '\x01');
          local_24 = local_c;
        }
        iVar8 = iVar8 + -1;
        iVar14 = iVar14 + -0x11;
      } while (0 < iVar8);
    }
    local_24 = local_24 + 1;
    uVar10 = local_18;
    if (local_24 < *(uint *)sizeHelp_exref) {
      uVar12 = local_24 * 0x11;
      local_c = uVar12;
      do {
        iVar8 = *(int *)(*(int *)(this_00 + 0x1c7) + 4 + uVar12);
        puVar2 = (undefined4 *)(*(int *)(this_00 + 0x1c7) + uVar12);
        local_c = uVar12;
        if (iVar8 == *local_14) {
          local_3c = *puVar2;
          local_38 = puVar2[1];
          local_34 = puVar2[2];
          local_30 = puVar2[3];
          local_2c = *(undefined1 *)(puVar2 + 4);
          local_2b = *(char *)((int)local_14 + 0x11) + '\x01';
          local_2a = 0;
          local_29 = 0;
          FUN_006b11d0(*(uint **)(this_00 + 0x1b3),uVar10 & 0xffff,&local_3c);
          uVar10 = uVar10 + 1;
          local_5 = '\x01';
          local_18 = uVar10;
        }
        else {
          if (iVar8 == 0) break;
          bVar6 = false;
          bVar7 = 0;
          local_6 = 0;
          local_28 = local_28 & 0xffffff00;
          if ((byte)local_10 != 0) {
            do {
              if (local_d0[local_28 & 0xff] == iVar8) {
                bVar6 = true;
                goto LAB_00513233;
              }
              bVar7 = bVar7 + 1;
              local_28 = CONCAT31(local_28._1_3_,bVar7);
            } while (bVar7 < (byte)local_10);
            bVar6 = false;
          }
LAB_00513233:
          if (bVar6) break;
        }
        local_24 = local_24 + 1;
        uVar12 = uVar12 + 0x11;
        local_c = uVar12;
      } while (local_24 < *(uint *)sizeHelp_exref);
    }
    piVar13 = local_14;
    if (local_5 != '\0') {
      iVar8 = *(int *)(this_00 + 0x1b3);
      if ((uVar10 & 0xffff) - 1 < *(uint *)(iVar8 + 0xc)) {
        iVar8 = ((uVar10 & 0xffff) - 1) * *(int *)(iVar8 + 8) + *(int *)(iVar8 + 0x1c);
      }
      else {
        iVar8 = 0;
      }
      if (iVar8 != 0) {
        *(undefined1 *)(iVar8 + 0x13) = 1;
      }
    }
  }
  else {
    iVar8 = *(int *)(local_20 + 0x1b3);
    uVar10 = param_2 + 1;
    uVar12 = *(uint *)(iVar8 + 0xc);
    while (((uVar10 < uVar12 &&
            (iVar9 = *(int *)(iVar8 + 8) * uVar10 + *(int *)(iVar8 + 0x1c), iVar9 != 0)) &&
           (*(byte *)((int)piVar13 + 0x11) < *(byte *)(iVar9 + 0x11)))) {
      FUN_006b0c70(iVar8,uVar10);
      iVar8 = *(int *)(this_00 + 0x1b3);
      uVar12 = *(uint *)(iVar8 + 0xc);
    }
  }
  *(bool *)((int)piVar13 + 0x12) = *(char *)((int)piVar13 + 0x12) == '\0';
  if ((*(int *)(this_00 + 0x19c) != 0) && (this_00[0x1a1] == (HelpPanelTy)0x0)) {
    FUN_006b55f0(*(undefined4 **)(this_00 + 0x68),0,0x21,0x16,*(int *)(this_00 + 0x1dc),0,0x21,0x16,
                 0x19c,0x118);
    pHVar1 = this_00 + 0x18;
    pHVar15 = pHVar1;
    for (iVar8 = 8; iVar8 != 0; iVar8 = iVar8 + -1) {
      *(undefined4 *)pHVar15 = 0;
      pHVar15 = pHVar15 + 4;
    }
    uVar4 = *(undefined4 *)(this_00 + 0x1b7);
    *(undefined4 *)(this_00 + 0x28) = 0x28;
    *(undefined2 *)(this_00 + 0x2c) = *(undefined2 *)(*(int *)(this_00 + 0x1b3) + 0xc);
    *(undefined2 *)(this_00 + 0x30) = 1;
    FUN_006e6080(this_00,2,*(undefined4 *)(this_00 + 0x19c),(undefined4 *)pHVar1);
    *(undefined2 *)(this_00 + 0x30) = 1;
    *(undefined2 *)(this_00 + 0x32) = 1;
    *(undefined4 *)(this_00 + 0x28) = 0x22;
    *(short *)(this_00 + 0x2c) = (short)uVar4;
    FUN_006e6080(this_00,2,*(undefined4 *)(this_00 + 0x19c),(undefined4 *)pHVar1);
    *(undefined4 *)(this_00 + 0x28) = 5;
    FUN_006e6080(this_00,2,*(undefined4 *)(this_00 + 0x19c),(undefined4 *)pHVar1);
  }
  DAT_00858df8 = (undefined4 *)local_80;
  return;
}

