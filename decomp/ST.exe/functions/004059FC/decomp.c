
void __thiscall WaitTy::ShowDescription(WaitTy *this)

{
  WaitTy *pWVar1;
  code *pcVar2;
  undefined4 *puVar3;
  int iVar4;
  ushort *puVar5;
  uint *puVar6;
  undefined4 uVar7;
  undefined4 uVar8;
  int iVar9;
  cMf32 *this_00;
  undefined4 unaff_ESI;
  WaitTy *this_01;
  void *unaff_EDI;
  byte *pbVar10;
  byte abStack_1a8 [260];
  undefined4 **ppuStack_a4;
  undefined4 auStack_a0 [16];
  undefined4 **ppuStack_60;
  undefined4 auStack_5c [16];
  WaitTy WStack_1c;
  undefined4 uStack_1b;
  undefined4 *puStack_10;
  int iStack_c;
  WaitTy *pWStack_8;
  
  iStack_c = 0;
  ppuStack_60 = DAT_00858df8;
  DAT_00858df8 = &ppuStack_60;
  pWStack_8 = this;
  iVar4 = __setjmp3(auStack_5c,0,unaff_EDI,unaff_ESI);
  if (iVar4 != 0) {
    DAT_00858df8 = ppuStack_60;
    iVar9 = FUN_006ad4d0(s_E____titans_Start_wait_obj_cpp_007cdd5c,0x15e,0,iVar4,&DAT_007a4ccc);
    if (iVar9 == 0) {
      FUN_006a5e40(iVar4,0,0x7cdd5c,0x15e);
      return;
    }
    pcVar2 = (code *)swi(3);
    (*pcVar2)();
    return;
  }
  puVar5 = FUN_0070a5a0(DAT_00806780,1,s_MM_MAPB_007cc790,0,1);
  thunk_FUN_00540760(*(undefined4 **)(DAT_0081176c + 0x2f0),0,0,'\x01',(byte *)puVar5);
  iVar4 = *(int *)(DAT_0081176c + 0x544);
  FUN_006b4170(iVar4,0,0,0,*(int *)(iVar4 + 4),*(int *)(iVar4 + 8),0xff);
  if (*(byte **)(DAT_0081176c + 0x548) != (byte *)0x0) {
    FUN_006b5570(*(byte **)(DAT_0081176c + 0x548));
  }
  puVar6 = FUN_006b54f0((uint *)0x0,10,10);
  this_01 = pWStack_8;
  *(uint **)(DAT_0081176c + 0x548) = puVar6;
  if (*(int *)(pWStack_8 + 0x1a8f) == -1) {
    uVar7 = FUN_006b0140(0x252c,DAT_00807618);
    uVar8 = FUN_006b0140(0x252a,DAT_00807618);
    wsprintfA((LPSTR)&DAT_0080f33a,s__s___s__007c85fc,uVar8,uVar7);
    iVar4 = 0;
    puVar6 = &DAT_0080f33a;
  }
  else {
    if (((byte)pWStack_8[0x1a93] < 2) || (3 < (byte)pWStack_8[0x1a93])) goto LAB_005e829d;
    wsprintfA((LPSTR)&DAT_0080f33a,&DAT_007c6ee4,&DAT_00807680,PTR_s_CUSTOM__0079c244);
    WStack_1c = this_01[0x1a93];
    uStack_1b = *(undefined4 *)(this_01 + 0x1a8f);
    iVar4 = FUN_00725910((char *)&DAT_0080f33a,s___DKD_007cce44,0,&LAB_00404e49,&WStack_1c,0);
    if (iVar4 == -0x70) {
      ppuStack_a4 = DAT_00858df8;
      DAT_00858df8 = &ppuStack_a4;
      iVar4 = __setjmp3(auStack_a0,0,unaff_EDI,unaff_ESI);
      if (iVar4 == 0) {
        puStack_10 = FUN_006f0ec0(0x345,&DAT_0080ed16,0,0,0);
      }
      this_01 = pWStack_8;
      puVar3 = puStack_10;
      DAT_00858df8 = ppuStack_a4;
      if (puStack_10 != (undefined4 *)0x0) {
        StartSystemTy::LoadMapData
                  (DAT_0081176c,(int)puStack_10,(pWStack_8[0x1a93] != (WaitTy)0x2) + '\x04');
        iStack_c = 1;
        FUN_0072e730(&DAT_0080ed16,(byte *)0x0,(byte *)0x0,abStack_1a8,(byte *)0x0);
        pbVar10 = abStack_1a8;
        uVar7 = FUN_006b0140(0x252a,DAT_00807618);
        wsprintfA((LPSTR)&DAT_0080f33a,s__s___s__007c85fc,uVar7,pbVar10);
        AddStr(this_01,&DAT_0080f33a,0);
        cMf32::delete(this_00,puVar3);
      }
      goto LAB_005e829d;
    }
    uVar7 = FUN_006b0140(0x252b,DAT_00807618);
    uVar8 = FUN_006b0140(0x252a,DAT_00807618);
    wsprintfA((LPSTR)&DAT_0080f33a,s__s___s__007c85fc,uVar8,uVar7);
    AddStr(this_01,&DAT_0080f33a,0);
    iVar4 = 0;
    puVar6 = (uint *)FUN_006b0140(0x252d,DAT_00807618);
  }
  AddStr(this_01,puVar6,iVar4);
LAB_005e829d:
  DAT_008087be = *(undefined4 *)(this_01 + 0x1a8f);
  if (iStack_c == 0) {
    *(undefined4 *)(this_01 + 0x2d) = 0x28;
    *(undefined2 *)(this_01 + 0x31) = 1;
    pWVar1 = this_01 + 0x1d;
    *(undefined2 *)(this_01 + 0x33) = *(undefined2 *)(*(int *)(DAT_0081176c + 0x548) + 8);
    FUN_006e6080(this_01,2,*(undefined4 *)(DAT_0081176c + 0x389),(undefined4 *)pWVar1);
    *(undefined4 *)(this_01 + 0x2d) = 0x22;
    *(undefined2 *)(this_01 + 0x33) = 0;
    *(undefined2 *)(this_01 + 0x31) = 0;
    FUN_006e6080(this_01,2,*(undefined4 *)(DAT_0081176c + 0x389),(undefined4 *)pWVar1);
    *(undefined4 *)(this_01 + 0x2d) = 0x20;
    *(undefined4 *)(this_01 + 0x31) = 0;
    FUN_006e6080(this_01,2,*(undefined4 *)(DAT_0081176c + 0x389),(undefined4 *)pWVar1);
  }
  if (-1 < (int)*(uint *)(DAT_0081176c + 0x2ec)) {
    FUN_006b35d0(DAT_008075a8,*(uint *)(DAT_0081176c + 0x2ec));
  }
  if (-1 < (int)*(uint *)(DAT_0081176c + 0x540)) {
    FUN_006b35d0(DAT_008075a8,*(uint *)(DAT_0081176c + 0x540));
  }
  DAT_00858df8 = ppuStack_60;
  return;
}

