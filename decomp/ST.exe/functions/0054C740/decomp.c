
void FUN_0054c740(void)

{
  code *pcVar1;
  int iVar2;
  void *this;
  int iVar3;
  byte bVar4;
  undefined4 unaff_EBX;
  void *unaff_ESI;
  undefined4 local_50;
  undefined4 local_4c [16];
  uint *local_c;
  uint local_8;
  
  local_50 = DAT_00858df8;
  DAT_00858df8 = &local_50;
  iVar2 = __setjmp3(local_4c,0,unaff_ESI,unaff_EBX);
  if (iVar2 == 0) {
    this = (void *)FUN_0072e530(0xf8);
    if (this == (void *)0x0) {
      DAT_00802a38 = (int *)0x0;
    }
    else {
      DAT_00802a38 = thunk_FUN_0054cb40(this,DAT_00806728);
    }
    if (DAT_00802a38 == (int *)0x0) {
      FUN_006a5e40(-1,DAT_007ed77c,0x7c8430,0x7d);
    }
    DAT_00802a38[0x38] = 0;
    (**(code **)*DAT_00802a38)();
    (**(code **)(*DAT_00802a38 + 8))(0x1100,0,&DAT_007fa174,0,0);
    (**(code **)(*DAT_00802a38 + 8))(0x109,0,0,0,0);
    (**(code **)(*DAT_00802a38 + 8))(0x121,0,0,0,0);
    (**(code **)(*DAT_00802a38 + 8))(0x143,&DAT_00811794,0,0,0);
    (**(code **)(*DAT_00802a38 + 8))(0x145,&DAT_008117a0,0,0,0);
    (**(code **)(*DAT_00802a38 + 8))(0x146,&DAT_008117ac,0,0,0);
    (**(code **)(*DAT_00802a38 + 8))(0x15b,&DAT_00811780,0,0,0);
    thunk_FUN_0058d6f0();
    if (DAT_0080879c == 0) {
      bVar4 = 0;
      local_8 = local_8 & 0xffffff00;
      do {
        thunk_FUN_00435850(local_8,1,(int *)0x0);
        bVar4 = bVar4 + 1;
        local_8 = CONCAT31(local_8._1_3_,bVar4);
      } while (bVar4 < 8);
    }
    thunk_FUN_00648fe0();
    if (DAT_00806754 != 0) {
      local_c = &local_8;
      local_8 = 0x32;
      FUN_006f1ce0(0xc,PTR_s_LAST_NAME_0079ae2c,(int *)&local_c,0);
      if ((uint)DAT_00802a38[8] < local_8) {
        DAT_00802a38[8] = local_8;
      }
    }
    FUN_006e4960(DAT_0081163c,DAT_00802a38,0);
    DAT_00802a38[0x38] = 1;
    if (DAT_008067a0 != '\0') {
      DAT_00802a99 = 1;
    }
    DAT_00858df8 = (undefined4 *)local_50;
    return;
  }
  DAT_00858df8 = (undefined4 *)local_50;
  iVar3 = FUN_006ad4d0(s_E____titans_Andrey_tplaysys_cpp_007c8430,0xb0,0,iVar2,&DAT_007a4ccc);
  if (iVar3 != 0) {
    pcVar1 = (code *)swi(3);
    (*pcVar1)();
    return;
  }
  FUN_006a5e40(iVar2,0,0x7c8430,0xb1);
  return;
}

