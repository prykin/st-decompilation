
void __thiscall MReportTy::PaintTab(MReportTy *this,int param_1)

{
  int iVar1;
  uint uVar2;
  code *pcVar3;
  uint *puVar4;
  int iVar5;
  int iVar6;
  int iVar7;
  undefined1 uVar8;
  char cVar9;
  undefined4 unaff_ESI;
  void *unaff_EDI;
  int iVar10;
  byte bVar11;
  InternalExceptionFrame IStack_8c;
  int iStack_48;
  uint *puStack_44;
  int iStack_40;
  int iStack_3c;
  int iStack_38;
  int iStack_34;
  int iStack_30;
  int iStack_2c;
  int iStack_28;
  int iStack_24;
  int iStack_20;
  int iStack_1c;
  int iStack_18;
  int iStack_14;
  int iStack_10;
  uint uStack_c;
  MReportTy *pMStack_8;
  
  puStack_44 = *(uint **)(param_1 + 0x14);
  iStack_40 = puStack_44[4] - 0x46;
  iStack_48 = puStack_44[3] - 0x1a;
  IStack_8c.previous = g_currentExceptionFrame;
  g_currentExceptionFrame = &IStack_8c;
  pMStack_8 = this;
  iVar5 = Library::MSVCRT::__setjmp3(IStack_8c.jumpBuffer,0,unaff_EDI,unaff_ESI);
  puVar4 = puStack_44;
  if (iVar5 != 0) {
    g_currentExceptionFrame = IStack_8c.previous;
    iVar7 = ReportDebugMessage(s_E____titans_Start_rpt_obj_cpp_007ccec8,0x369,0,iVar5,&DAT_007a4ccc,
                               s_MReportTy__PaintTab_007cd084);
    if (iVar7 == 0) {
      RaiseInternalException(iVar5,0,s_E____titans_Start_rpt_obj_cpp_007ccec8,0x369);
      return;
    }
    pcVar3 = (code *)swi(3);
    (*pcVar3)();
    return;
  }
  FUN_006b5f80(DAT_008075a8,puStack_44[3],puStack_44[4],puStack_44[5],puStack_44[6]);
  iVar7 = iStack_40;
  iVar5 = iStack_48;
  FUN_006b4170(*(int *)(pMStack_8 + 0x73),0,iStack_48,iStack_40,puVar4[5],puVar4[6],0xff);
  uVar2 = puVar4[1];
  if (uVar2 == 0) {
    iStack_3c = iVar5 + 5;
    iStack_38 = iVar7 + 2;
    iStack_34 = iVar5 + 7;
    iStack_30 = iVar7;
    iStack_28 = iVar7;
    iStack_2c = iVar5 + -3 + puVar4[5];
    iStack_24 = iVar5 + -3 + puVar4[5];
    iStack_20 = (puVar4[6] - 1) + iVar7;
    iStack_18 = (puVar4[6] - 1) + iVar7;
    iStack_10 = (puVar4[6] - 3) + iVar7;
    iStack_1c = iStack_34;
    iStack_14 = iStack_3c;
    FUN_006c7ea0(*(undefined4 *)(pMStack_8 + 0x73),0,&iStack_3c,6,0x4c);
    FUN_006c7f10(*(int *)(pMStack_8 + 0x73),0,&iStack_3c,6,0x18);
    iVar6 = iVar5 + -1 + puVar4[5];
LAB_005c17e9:
    bVar11 = 0x18;
    iVar1 = puVar4[6] - 1;
    iVar10 = iVar7;
  }
  else {
    if (uVar2 != 1) {
      if (uVar2 != 2) goto LAB_005c1806;
      iVar6 = iVar5 + -1 + puVar4[5];
      goto LAB_005c17e9;
    }
    iStack_38 = iVar7 + 2;
    iStack_34 = iVar5 + 2;
    iStack_3c = iVar5;
    iStack_30 = iVar7;
    iStack_28 = iVar7;
    iStack_2c = iVar5 + -1 + puVar4[5];
    iStack_24 = iVar5 + -1 + puVar4[5];
    iStack_20 = (puVar4[6] - 1) + iVar7;
    iStack_14 = iVar5;
    iStack_18 = (puVar4[6] - 1) + iVar7;
    iStack_10 = (puVar4[6] - 3) + iVar7;
    iStack_1c = iStack_34;
    FUN_006c7ea0(*(undefined4 *)(pMStack_8 + 0x73),0,&iStack_3c,6,0x4c);
    FUN_006c7f10(*(int *)(pMStack_8 + 0x73),0,&iStack_3c,6,0x18);
    bVar11 = 0x4c;
    iVar6 = iVar5 + -1 + puVar4[5];
    iVar1 = puVar4[6] - 2;
    iVar10 = iVar7 + 1;
  }
  FUN_006b5b10(*(int *)(pMStack_8 + 0x73),0,iVar6,iVar10,iVar6,iVar1 + iVar7,bVar11,0xd);
LAB_005c1806:
  if (puVar4[1] < 2) {
    uVar2 = *puVar4;
    cVar9 = -1;
    uStack_c = CONCAT31(uStack_c._1_3_,0xff);
    if (1 < uVar2) {
      cVar9 = DAT_0080c846;
      if (uVar2 != 2) {
        cVar9 = *(char *)((int)&DAT_0080c83a + uVar2 + 2);
      }
      uStack_c = CONCAT31(uStack_c._1_3_,cVar9);
    }
    if (cVar9 != -1) {
      switch(uStack_c & 0xff) {
      case 0:
        uVar8 = 0xfc;
        break;
      case 1:
        uVar8 = 0xfa;
        break;
      case 2:
        uVar8 = 0xfb;
        break;
      case 3:
        uVar8 = 0xf9;
        break;
      case 4:
        uVar8 = 0xfd;
        break;
      case 5:
        uVar8 = 0xfe;
        break;
      case 6:
        uVar8 = 0xf3;
        break;
      case 7:
        uVar8 = 7;
        break;
      default:
        uVar8 = 0xff;
      }
      FUN_006b4170(*(int *)(pMStack_8 + 0x73),0,(-(uint)(puVar4[1] != 0) & 0xfffffffb) + 0xc + iVar5
                   ,iVar7 + 0xb,0x1a,0xd,uVar8);
      FUN_006b5ee0(*(int *)(pMStack_8 + 0x73),0,(-(uint)(puVar4[1] != 0) & 0xfffffffb) + 0xc + iVar5
                   ,iVar7 + 0xb,0x1a,0xd,0,0xd);
    }
  }
  Library::DKW::DDX::FUN_006b48e0
            (DAT_0080759c,puVar4[3],puVar4[4],*(int *)(pMStack_8 + 0x73),0,iVar5,iVar7,puVar4[5],
             puVar4[6],(int)(pMStack_8 + 0xa3),0x4c,0x10000ff);
  g_currentExceptionFrame = IStack_8c.previous;
  return;
}

