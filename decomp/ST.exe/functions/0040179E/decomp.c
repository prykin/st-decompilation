
/* WARNING: Globals starting with '_' overlap smaller symbols at the same address */

undefined4 __fastcall thunk_FUN_0060d940(int param_1)

{
  undefined4 uVar1;
  float fVar2;
  int iVar3;
  int iVar4;
  undefined4 *puVar5;
  uint uVar6;
  int iVar7;
  float10 fVar8;
  unkbyte10 extraout_ST1;
  float10 extraout_ST1_00;
  longlong lVar9;
  int iStack_c;
  int iStack_8;
  
  fVar8 = (float10)_DAT_0079034c;
  iVar4 = *(int *)(param_1 + 0x244);
  if (iVar4 == 0) {
    return 0;
  }
  iStack_8 = 0;
  fVar2 = _DAT_0079c5a4 / (float)*(int *)(iVar4 + 0xc);
  if (0 < *(int *)(iVar4 + 0xc)) {
    do {
      fcos(fVar8 + (float10)fVar2);
      iVar4 = *(int *)(iVar4 + 0x10);
      uVar6 = iVar4 * iStack_8;
      lVar9 = Library::MSVCRT::__ftol();
      fsin(extraout_ST1);
      iVar7 = (int)lVar9 + *(int *)(param_1 + 0x1e9);
      lVar9 = Library::MSVCRT::__ftol();
      iVar3 = (int)lVar9 + *(int *)(param_1 + 0x1ed);
      iStack_c = 0;
      if (0 < iVar4) {
        do {
          iVar4 = *(int *)(param_1 + 0x234);
          if ((uVar6 < *(uint *)(iVar4 + 0xc)) &&
             (puVar5 = (undefined4 *)(*(int *)(iVar4 + 8) * uVar6 + *(int *)(iVar4 + 0x1c)),
             puVar5 != (undefined4 *)0x0)) {
            puVar5[6] = iVar7;
            puVar5[7] = iVar3;
            uVar1 = *(undefined4 *)(param_1 + 0x1f1);
            puVar5[3] = iVar7;
            puVar5[0xb] = (float)extraout_ST1_00;
            puVar5[8] = uVar1;
            puVar5[4] = iVar3;
            puVar5[5] = uVar1;
            puVar5[10] = 0x14;
            puVar5[9] = 0;
            *puVar5 = *(undefined4 *)(param_1 + 0x211);
          }
          iStack_c = iStack_c + 1;
          uVar6 = uVar6 + 1;
        } while (iStack_c < *(int *)(*(int *)(param_1 + 0x244) + 0x10));
      }
      iVar4 = *(int *)(param_1 + 0x244);
      iStack_8 = iStack_8 + 1;
      fVar8 = extraout_ST1_00;
    } while (iStack_8 < *(int *)(iVar4 + 0xc));
  }
  return 1;
}

