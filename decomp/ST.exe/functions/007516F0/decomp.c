#include "../../pseudocode_runtime.h"


/* WARNING: Function: __alloca_probe replaced with injection: alloca_probe */
/* WARNING: Removing unreachable block (ram,0x00751954) */

int FUN_007516f0(RecoveredRecord_007516F0_6354DBDD *param_1,byte *param_2)

{
  byte bVar1;
  byte bVar2;
  uint uVar3;
  int iVar4;
  uint uVar5;
  uint uVar6;
  RecoveredRecord_007516F0_6354DBDD *pRVar7;
  byte *pbVar8;
  byte abStack_10060 [65544];
  uint local_4c;
  byte local_48;
  byte local_44;
  RecoveredRecord_007516F0_6354DBDD *local_3c;
  byte *local_24;

  pbVar8 = abStack_10060;
  for (iVar4 = 0x4000; iVar4 != 0; iVar4 = iVar4 + -1) {
    pbVar8[0] = 0;
    pbVar8[1] = 0;
    pbVar8[2] = 0;
    pbVar8[3] = 0;
    pbVar8 = pbVar8 + 4;
  }
  if (abStack_10060 == nullptr) {
    iVar4 = -2;
  }
  else {
    local_24 = param_2;
    local_44 = param_1->field_0001;
    local_48 = 0;
    uVar3 = 0;
    uVar6 = 0;
    pRVar7 = param_1 + 1;
    local_3c = param_1 + 1;
    while( true ) {
      if ((local_44 & (byte)(1 << (local_48 & 0x1f))) == 0) {
        bVar1 = *(byte *)pRVar7;
        pRVar7 = (RecoveredRecord_007516F0_6354DBDD *)&pRVar7->field_0001;
        abStack_10060[uVar6 << 8 | uVar3] = bVar1;
        local_3c = pRVar7;
      }
      else {
        bVar1 = abStack_10060[uVar6 << 8 | uVar3];
      }
      uVar5 = (uint)bVar1;
      local_48 = local_48 + 1;
      if (7 < local_48) {
        local_44 = *(byte *)pRVar7;
        pRVar7 = (RecoveredRecord_007516F0_6354DBDD *)&pRVar7->field_0001;
        local_48 = 0;
        local_3c = pRVar7;
      }
      if (uVar5 == 0) break;
      if ((bVar1 & 0x40) == 0) {
        local_4c = uVar5 & 0x3f;
        uVar6 = uVar5;
        uVar5 = uVar3;
      }
      else {
        if ((local_44 & (byte)(1 << (local_48 & 0x1f))) == 0) {
          bVar2 = *(byte *)pRVar7;
          pRVar7 = (RecoveredRecord_007516F0_6354DBDD *)&pRVar7->field_0001;
          abStack_10060[uVar3 << 8 | uVar5] = bVar2;
          local_3c = pRVar7;
        }
        else {
          bVar2 = abStack_10060[uVar3 << 8 | uVar5];
        }
        local_48 = local_48 + 1;
        if (7 < local_48) {
          local_44 = *(byte *)pRVar7;
          pRVar7 = (RecoveredRecord_007516F0_6354DBDD *)&pRVar7->field_0001;
          local_48 = 0;
          local_3c = pRVar7;
        }
        local_4c = (uint)bVar2 | (uVar5 & 0x3f) << 8;
        uVar6 = (uint)bVar2;
      }
      if ((bVar1 & 0x80) == 0) {
        if ((local_44 & (byte)(1 << (local_48 & 0x1f))) == 0) {
          bVar1 = *(byte *)pRVar7;
          pRVar7 = (RecoveredRecord_007516F0_6354DBDD *)&pRVar7->field_0001;
          abStack_10060[uVar5 << 8 | uVar6] = bVar1;
          local_3c = pRVar7;
        }
        else {
          bVar1 = abStack_10060[uVar5 << 8 | uVar6];
        }
        uVar3 = (uint)bVar1;
        local_48 = local_48 + 1;
        if (7 < local_48) {
          local_44 = *(byte *)pRVar7;
          local_3c = (RecoveredRecord_007516F0_6354DBDD *)&pRVar7->field_0001;
          local_48 = 0;
        }
        pbVar8 = local_24;
        for (uVar5 = local_4c >> 2; uVar5 != 0; uVar5 = uVar5 - 1) {
          *(uint *)pbVar8 = CONCAT22(CONCAT11(bVar1,bVar1),CONCAT11(bVar1,bVar1));
          pbVar8 = pbVar8 + 4;
        }
        for (uVar5 = local_4c & 3; uVar5 != 0; uVar5 = uVar5 - 1) {
          *pbVar8 = bVar1;
          pbVar8 = pbVar8 + 1;
        }
        local_24 = local_24 + local_4c;
        pRVar7 = local_3c;
      }
      else {
        uVar3 = uVar6;
        uVar6 = uVar5;
        if (-1 < (int)(local_4c - 1)) {
          do {
            uVar6 = uVar3;
            if ((local_44 & (byte)(1 << (local_48 & 0x1f))) == 0) {
              bVar1 = *(byte *)pRVar7;
              pRVar7 = (RecoveredRecord_007516F0_6354DBDD *)&pRVar7->field_0001;
              abStack_10060[uVar5 << 8 | uVar6] = bVar1;
            }
            else {
              bVar1 = abStack_10060[uVar5 << 8 | uVar6];
            }
            uVar3 = (uint)bVar1;
            local_48 = local_48 + 1;
            if (7 < local_48) {
              local_44 = *(byte *)pRVar7;
              pRVar7 = (RecoveredRecord_007516F0_6354DBDD *)&pRVar7->field_0001;
              local_48 = 0;
            }
            *local_24 = bVar1;
            local_24 = local_24 + 1;
            local_4c = local_4c - 1;
            uVar5 = uVar6;
            local_3c = pRVar7;
          } while (local_4c != 0);
        }
      }
    }
    iVar4 = (int)local_24 - (int)param_2;
  }
  return iVar4;
}

