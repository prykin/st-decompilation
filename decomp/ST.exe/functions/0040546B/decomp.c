
uint * __thiscall thunk_FUN_0049fbf0(void *this,int param_1)

{
  code *pcVar1;
  void *pvVar2;
  int iVar3;
  int *this_00;
  uint *puVar4;
  uint uVar5;
  void *pvVar6;
  uint uVar7;
  int iVar8;
  undefined4 unaff_ESI;
  uint *puVar9;
  void *unaff_EDI;
  undefined4 *puVar10;
  uint uVar11;
  undefined4 uStack_70;
  undefined4 auStack_6c [16];
  undefined4 uStack_2c;
  undefined2 uStack_28;
  undefined2 uStack_26;
  undefined2 uStack_24;
  void *pvStack_20;
  uint *puStack_1c;
  undefined4 uStack_18;
  uint *puStack_14;
  uint uStack_10;
  uint *puStack_c;
  uint uStack_8;
  
  uStack_10 = *(uint *)(*(int *)((int)this + 0x29) + 0xc);
  puStack_14 = (uint *)0x0;
  puStack_c = (uint *)0x0;
  puStack_1c = (uint *)0x2;
  uStack_70 = DAT_00858df8;
  DAT_00858df8 = &uStack_70;
  pvStack_20 = this;
  iVar3 = __setjmp3(auStack_6c,0,unaff_EDI,unaff_ESI);
  pvVar2 = pvStack_20;
  if (iVar3 != 0) {
    DAT_00858df8 = (undefined4 *)uStack_70;
    if (iVar3 == -0x5001fff7) {
      return puStack_1c;
    }
    iVar8 = FUN_006ad4d0(s_E____titans_wlad_to_grpb_cpp_007abe3c,0xcb0,0,iVar3,&DAT_007a4ccc);
    if (iVar8 != 0) {
      pcVar1 = (code *)swi(3);
      puVar4 = (uint *)(*pcVar1)();
      return puVar4;
    }
    FUN_006a5e40(iVar3,0,0x7abe3c,0xcb1);
    return (uint *)0xffffffff;
  }
  if ((param_1 != 0) && (param_1 != 1)) goto LAB_0049fe51;
  puVar10 = (undefined4 *)((int)pvStack_20 + 0x89);
  for (iVar3 = 0x15; iVar3 != 0; iVar3 = iVar3 + -1) {
    *puVar10 = 0;
    puVar10 = puVar10 + 1;
  }
  uVar11 = 0;
  *(undefined4 *)((int)pvStack_20 + 0x65) = 0;
  *(undefined4 *)((int)pvStack_20 + 0x2e8) = *(undefined4 *)((int)pvStack_20 + 0x1b3);
  *(undefined2 *)((int)pvStack_20 + 0x2ec) = *(undefined2 *)((int)pvStack_20 + 0x1b7);
  uStack_28 = *(undefined2 *)((int)pvStack_20 + 0x2e8);
  uStack_26 = *(undefined2 *)((int)pvStack_20 + 0x2ea);
  uStack_2c = *(undefined4 *)(DAT_00802a38 + 0xe4);
  uStack_24 = *(undefined2 *)((int)pvStack_20 + 0x2ec);
  uStack_18 = uStack_2c;
  if (0 < (int)uStack_10) {
    do {
      FUN_006acc70(*(int *)((int)pvVar2 + 0x29),uVar11,&uStack_8);
      if ((short)uStack_8 != -1) {
        this_00 = (int *)thunk_FUN_0042b620(CONCAT31((int3)(uStack_8 >> 8),
                                                     *(undefined1 *)((int)pvVar2 + 0x24)),uStack_8,1
                                           );
        if (this_00 == (int *)0x0) {
          FUN_006a5e40(-0x5001fffc,DAT_007ed77c,0x7abe3c,0xc79);
        }
        iVar3 = (**(code **)(*this_00 + 0x2c))();
        if (((iVar3 == 7) || (iVar3 = (**(code **)(*this_00 + 0x2c))(), iVar3 == 0x13)) ||
           (iVar3 = (**(code **)(*this_00 + 0x2c))(), iVar3 == 0x1b)) {
          thunk_FUN_0045ef00(this_00,0x17,&uStack_2c);
          puVar4 = puStack_14;
          if (puStack_14 == (uint *)0x0) {
            puVar4 = FUN_006ae290((uint *)0x0,1,2,1);
            puStack_14 = puVar4;
          }
        }
        else {
          iVar3 = thunk_FUN_00490d90((int)this_00);
          if (iVar3 == 0) {
            thunk_FUN_0045ef00(this_00,3,&uStack_18);
            goto LAB_0049fd94;
          }
          puVar4 = puStack_c;
          if (puStack_c == (uint *)0x0) {
            puVar4 = FUN_006ae290((uint *)0x0,1,2,1);
            puStack_c = puVar4;
          }
        }
        FUN_006ae1c0(puVar4,&uStack_8);
      }
LAB_0049fd94:
      uVar11 = uVar11 + 1;
    } while ((int)uVar11 < (int)uStack_10);
  }
  puVar9 = puStack_c;
  puVar4 = puStack_14;
  if ((puStack_14 == (uint *)0x0) || (puStack_c == (uint *)0x0)) {
    if (puStack_14 == (uint *)0x0) {
      if (puStack_c != (uint *)0x0) {
        uVar11 = puStack_c[3];
        uVar7 = 0;
        uStack_10 = uVar11;
        if (0 < (int)uVar11) {
          do {
            uVar5 = FUN_006acc70((int)puStack_c,uVar7,&uStack_8);
            pvVar6 = (void *)thunk_FUN_0042b620(CONCAT31((int3)(uVar5 >> 8),
                                                         *(undefined1 *)((int)pvVar2 + 0x24)),
                                                uStack_8,1);
            thunk_FUN_0045ef00(pvVar6,3,&uStack_18);
            uVar7 = uVar7 + 1;
          } while ((int)uVar7 < (int)uVar11);
        }
        puStack_1c = (uint *)0x0;
      }
      puVar9 = puStack_c;
      puVar4 = puStack_14;
      if (puStack_14 == (uint *)0x0) {
        if (puStack_c == (uint *)0x0) {
          puStack_1c = puStack_c;
        }
        goto LAB_0049fe22;
      }
    }
LAB_0049fe26:
    FUN_006ae110((byte *)puVar4);
  }
  else {
    thunk_FUN_00445a40(CONCAT31((int3)(uStack_10 >> 8),*(undefined1 *)((int)pvVar2 + 0x24)),
                       puStack_14,puStack_c);
LAB_0049fe22:
    if (puVar4 != (uint *)0x0) goto LAB_0049fe26;
  }
  if (puVar9 != (uint *)0x0) {
    FUN_006ae110((byte *)puVar9);
  }
  FUN_006a5e40(-0x5001fff7,DAT_007ed77c,0x7abe3c,0xca0);
LAB_0049fe51:
  if (*(uint *)(DAT_00802a38 + 0xe4) % 0x19 == 7) {
    uVar11 = 0;
    if (0 < (int)uStack_10) {
      do {
        FUN_006acc70(*(int *)((int)pvVar2 + 0x29),uVar11,&uStack_8);
        if ((short)uStack_8 != -1) {
          pvVar6 = (void *)thunk_FUN_0042b620(CONCAT31((int3)(uStack_8 >> 8),
                                                       *(undefined1 *)((int)pvVar2 + 0x24)),uStack_8
                                              ,1);
          if (pvVar6 == (void *)0x0) {
            FUN_006a5e40(-0x5001fffc,DAT_007ed77c,0x7abe3c,0xca7);
          }
          iVar3 = thunk_FUN_0045ff10((int)pvVar6);
          if ((iVar3 == 0x17) || (uVar7 = thunk_FUN_0045f400(pvVar6,0x17), uVar7 == 1)) break;
        }
        uVar11 = uVar11 + 1;
      } while ((int)uVar11 < (int)uStack_10);
    }
    if (uVar11 == uStack_10) {
      puStack_1c = (uint *)0x0;
    }
  }
  DAT_00858df8 = (undefined4 *)uStack_70;
  return puStack_1c;
}

