...
Generated-layout hashes are computed from the installed `StructureDB` after
`replaceWith`, not from the transient `StructureDataType`. Ghidra may
canonicalize default or overlapping components while installing a structure;
hashing before installation makes untouched generated output look manually
edited. For legacy script-owned classes already affected by that mismatch, the
proposal column `repair_mode=surgical` authorizes only exact component changes
whose current type and `[STClassLayoutApplier]` provenance still match the
recorded field baseline. The applier preflights every target datatype, preserves
all unrelated components, and records a separate overlay hash. Surgical mode
never authorizes a whole-layout rebuild or treats a missing nested type as a
partial success.
...
### 5.1. Adjacent jump/lookup-table boundaries

`STJumpTableBoundaryAnalyzer` runs before any broad decompiler-backed recovery
pass. It scans machine instructions for computed jumps whose existing jump
references exactly match a consecutive dword table of instruction starts in the
containing function. The first rejected dword must be non-executable and its
same address must be independently used as the base of an indexed byte load.
This identifies the MSVC layout in which a dword destination table is followed
immediately by a byte selector table without confusing the two arrays.

Machine geometry alone is retained as review evidence. Automatic application
additionally requires a fresh Ghidra decompile to emit `Sanity check requires
truncation of jumptable`, or an existing script-owned override which needs
idempotent revalidation. `STJumpTableBoundaryApplier` rechecks every physical
dword, reference, instruction target, following word, and byte-read site inside
one transaction. A different unowned/manual override is preserved.

The analyzer writes
`recovery/ST.exe/jump_table_boundary_proposals.tsv`; the applier consumes that
exact file and writes `jump_table_boundary_apply_report.tsv`. This is a database
repair using Ghidra's normal `JumpTable.writeOverride`, not an exporter warning
suppression.
...
19. Run `STGlobalDataAnalyzer`.
   ...
   - Character-storage recovery follows the complete forward machine CFG from
     an exact `MOV EDI,<global>` to a shared `SCASB` tail. Every path must reach
     the scan with AL proven zero before a call, backward edge, function exit,
     or EDI overwrite. This covers switch cases which jump to one inlined
     `strlen` tail without weakening the proof to a mnemonic-distance guess.
   - Repeated exact dereference of one loaded global value proves a neutral
     `void *` only after at least three sites in two functions. The equivalent
     receiver rule requires three sites in two callers and two distinct
     `__thiscall` callees. Neither rule invents a pointee.
   - A concrete singleton class may be recovered from a named method only when
     forward CFG must-analysis carries the unadjusted ECX receiver through exact
     register or EBP-stack copies to the global store. Volatile registers die at
     calls and joins retain only unanimous facts. A trusted concrete-pointer
     function may instead type the singleton when every RET returns that exact
     global or null. Script-owned neutral pointers remain replaceable by later
     stronger evidence; manual/imported data does not.
...
26. Run `STIndirectCallApplier`.
    ...
    - Polymorphic dispatch interfaces and tail signatures are audit-only. The
      ...
      file enables them.
    - Then run `STIndirectCallsiteAnalyzer` with the recovery directory and
      `STIndirectCallsiteApplier` with
      `<repo>/recovery/ST.exe/indirect_callsite_proposals.tsv`. This pair types
      one indirect call instruction without widening its shared physical slot.
      Exact dispatch consensus is preferred; neutral machine fallback is
      suppressed for an entire function when it would require more than 32
      remaining per-call overrides after same-pass physical-slot promotion. A
      uniquely owned raw slot may instead become one fixed physical ABI when at
      least eight exact unadjusted calls in one dense function agree on arity
      and return role; the analyzer removes the redundant address-local rows in
      that same proposal. The applier removes only stale script-owned overrides
      and preserves foreign/manual ones.
...
The same pair repairs a common MSVC SEH/setjmp receiver spill without relying
on an image address, class name, or source path. A persistent Listing local must
trace to the function's exact auto-`this` value through only same-size neutral
...
A decompiler-only nominal spelling is not independent evidence for
`typed_copy`. An ordinary copied value must be rooted at an exact typed
parameter, global, return, or call boundary. A local-to-local or symbol-less
copy is accepted only when its complete pointer/type chain ends at the same
hash-intact recursive node; otherwise the apparent type can merely be the
constraint which the analyzer is trying to validate. When one Listing local is
split into several merge groups, the applier assigns deterministic
`<name>_mg<group>` database names (with a collision suffix when required). A
fresh decompile must reattach each generated local to the same machine anchor;
failed reattachment is preserved as a conflict rather than reported as applied.
...
| `STConstructorAnalyzer/Applier` | Recover constructors, allocation sizes, direct hierarchy evidence, receiver-only signatures, and ABI `Owner *` returns when EAX is proven to return `this`. A concrete factory result may anchor an otherwise unnamed constructor only through an exact allocation-size match, unadjusted allocation-result flow into ECX, and one terminal JMP/thunk chain, with no conflicting high-confidence vtable owner. |
...
| `STGlobalDataAnalyzer/Applier` | Type generic globals from receiver/argument use and named-constructor stores, recover bare initialized pointers to exact string `Data` as `char *`, preserve or reconstruct their target-derived `PTR_s_*` identity instead of call-parameter names, follow the constructor-result edge through MSVC new/null join blocks, accept ordinary `T **` address-taking when an unambiguous named-constructor store proves the singleton's `T *` value, promote script-owned anonymous singleton pointers to named classes or dominant statically linked library contexts, and prove neutral pointer role from repeated exact dereferences or independent untyped `__thiscall` receiver sites. A concrete singleton may additionally come from all-predecessor CFG propagation of an unadjusted named receiver into its exact global store, or from a trusted concrete-pointer function whose every return yields that global or null. The analyzer also proves narrow character storage from a dominant exact `char` call-boundary quorum or a complete bounded forward-CFG proof from `MOV EDI,global` to `REPNE SCASB` with AL zero on every path, and recovers a runtime bit-string global as `byte *` only when its exact loaded value is the memory base of repeated x86 `BT`/`BTS`/`BTR`/`BTC` instructions across at least two functions. It names literal-backed module handles, assigns address-stable structural names, and audits every `PTR_*` symbol by pointer role. Printable bytes alone never prove a string, and neutral pointer evidence never proves a pointee. |
...
| `STFunctionPointerFieldAnalyzer/Applier` | Recover non-vtable callback members only from the complete chain “exact stored function address → one generated structure field → indirect call through that same field.” The machine prepass accepts a destination only when the complete rendered operand is bracketed memory—Ghidra may also report its embedded base register as an operand object—and records every exact store in `function_pointer_field_machine_stores.tsv`. High p-code then joins the exact instruction address back to one generated field before call-only functions are decompiled. With no exact stored target, call-only decompilation and call-only proposal noise are skipped. All stored targets must have one imported or independently recovered ABI; bare `USER_DEFINED` provenance is review-only, and manual structures and concrete fields are preserved. The report retains all exact observed stores and their rejection reasons. |
| `STIndirectCallAnalyzer/Applier` | Audit raw indirect calls; refine trusted physical slots, install machine-proven neutral thiscall/stdcall definitions, and propagate an ABI only across unanimous independently generated slot occurrences of the same raw or resolved target. Automation-owned datatype category remains provenance when later parameter refinement has canonicalized away a function-definition comment. Weak structural vtable typedefs do not veto that family. Polymorphic dispatch interfaces are proposal-only, and the applier refuses legacy attempts to install them into class vptrs or synthetic tail slots. |
| `STIndirectCallsiteAnalyzer/Applier` | Install address-stable call overrides for exact physical dispatch or a bounded neutral machine fallback without widening the shared slot. Its CFG-aware receiver provenance follows convergent predecessors and callee-saved transport, and the audit partitions physical dispatch, stored callbacks, callback parameters, function tables, external interfaces, linked-library runtime, and unresolved transports. Dense machine fallback families above 32 sites per function are suppressed and previously installed script-owned members are cleaned up; exact consensus and foreign/manual overrides remain untouched. |
| `STCallableReceiverAnalyzer/Applier` | Recover a persistent structural receiver record only for a dense exact parameter-origin family with unanimous non-void slot ABIs, independently proven extent, no reuse of the parameter storage, and no unresolved incoming callee-saved argument. The deterministic type is a function-local callable view under `Recovered/PointerShapes/CallableReceivers`; it does not assert a semantic class, physical vtable, or inheritance. Mixed or unstable families remain in the proposal report. |
