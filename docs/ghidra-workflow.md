...
| `STIndirectCallsiteAnalyzer/Applier` | Install address-stable call overrides for exact physical dispatch or a bounded neutral machine fallback without widening the shared slot. Dense machine fallback families above 32 sites per function are suppressed and previously installed script-owned members are cleaned up; exact consensus and foreign/manual overrides remain untouched. |
