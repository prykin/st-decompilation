#include "st/generated.hpp"
// Generated translation unit: source/recovered/by_owner/SubmarineTitans__Recovered__HiddenThis__AnonReceiver_004603B0.cpp

// 004838E0 SubmarineTitans::Recovered::HiddenThis::AnonReceiver_004603B0::FUN_004838e0
#line 4 "decomp/ST.exe/functions/004838E0/decomp.c"
/* WARNING: Removing unreachable block (ram,0x00483caa) */
/* [STHiddenThisApplier] Anonymous hidden receiver recovered as
   /SubmarineTitans/Recovered/HiddenThis/AnonReceiver_004603B0.
   Evidence: incoming_receiver_captures=1; receiver_accesses=4; incoming_edx_uses=0; calls=3;
   ecx_pointer_setup=3; ecx_scalar_setup=0; caller_cleanup_calls=0; callee_ret_pop=[4];
   expected_stack=4; receiver_family_members=3; adopt_untyped_existing_thiscall */

undefined4 __thiscall
st::fn_004838E0
          (AnonReceiver_004603B0 *this,RecoveredRecord_STBoatC_004838E0 *param_1)

{
  short sVar1;
  short sVar2;
  short sVar3;
  uint uVar4;
  uint uVar5;
  uint uVar6;
  uint uVar7;
  uint uVar8;
  uint uVar9;
  int iVar10;
  uint uVar11;
  int iVar12;
  int iVar13;
  int iVar14;
  uint uVar15;
  uint uVar16;
  uint uVar17;
  uint uVar18;
  uint uVar19;
  uint uVar20;
  uint uVar21;
  longlong lVar22;
  longlong lVar23;
  longlong lVar24;
  undefined8 uVar25;
  longlong lVar26;
  longlong lVar27;
  longlong lVar28;
  longlong local_60;
  undefined8 local_48;

  sVar1 = param_1->field_003A;
  uVar16 = (uint)param_1->field_0034;
  uVar4 = (int)sVar1 - uVar16;
  uVar17 = (int)uVar4 >> 0x1f;
  sVar2 = param_1->field_003C;
  uVar18 = (uint)param_1->field_0036;
  uVar5 = (int)sVar2 - uVar18;
  uVar19 = (int)uVar5 >> 0x1f;
  sVar3 = param_1->field_003E;
  uVar20 = (uint)param_1->field_0038;
  uVar6 = (int)sVar3 - uVar20;
  uVar21 = (int)uVar6 >> 0x1f;
  uVar7 = (uint)*(short *)&this->field_0x45;
  uVar8 = (uint)*(short *)&this->field_0x43;
  uVar9 = (uint)*(short *)&this->field_0x41;
  lVar22 = st::fn_0072E4F0(uVar9,(int)uVar9 >> 0x1f,uVar4,uVar17);
  lVar23 = st::fn_0072E4F0(uVar8,(int)uVar8 >> 0x1f,uVar5,uVar19);
  lVar24 = st::fn_0072E4F0(uVar7,(int)uVar7 >> 0x1f,uVar6,uVar21);
  lVar24 = lVar24 + lVar23 + lVar22;
  iVar10 = (int)lVar24;
  uVar11 = -iVar10;
  uVar15 = -((int)((ulonglong)lVar24 >> 0x20) + (uint)(iVar10 != 0));
  iVar10 = (int)uVar16 >> 0x1f;
  if (uVar4 == 0) {
    uVar20 = uVar16;
    iVar14 = iVar10;
    if (uVar5 == 0) {
      local_48 = (longlong)(int)uVar18;
      if (uVar6 == 0) {
        return 0;
      }
      uVar25 = st::fn_0072E440(uVar11,uVar15,uVar6,uVar21);
      /* ST_PSEUDO[packed_or_unaligned_piece]: expected named packed member, bit extract/compose, or unaligned load */
      uVar25 = CONCAT44(-((int)((ulonglong)uVar25 >> 0x20) + (uint)((int)uVar25 != 0)),-(int)uVar25);
    }
    else {
      local_60 = (longlong)(int)uVar18;
      lVar24 = st::fn_0072E4F0(uVar18,(int)uVar18 >> 0x1f,uVar6,uVar21);
      lVar24 = st::fn_0072E4F0((uint)lVar24,(int)((ulonglong)lVar24 >> 0x20),uVar6,uVar21);
      lVar22 = st::fn_0072E4F0(uVar11,uVar15,uVar5,uVar19);
      lVar23 = st::fn_0072E4F0(uVar6,uVar21,uVar6,uVar21);
      lVar26 = st::fn_0072E4F0(uVar5,uVar19,uVar5,uVar19);
      local_48 = st::fn_0072E440
                           ((uint)(lVar24 - lVar22),(uint)((ulonglong)(lVar24 - lVar22) >> 0x20),
                            (uint)(lVar23 + lVar26),(uint)((ulonglong)(lVar23 + lVar26) >> 0x20));
      lVar24 = st::fn_0072E4F0
                         ((uint)(local_48 - local_60),
                          (int)((ulonglong)(local_48 - local_60) >> 0x20),uVar6,uVar21);
      uVar25 = st::fn_0072E440
                         ((uint)lVar24,(uint)((ulonglong)lVar24 >> 0x20),uVar5,uVar19);
    }
  }
  else {
    lVar23 = st::fn_0072E4F0(uVar20,(int)uVar20 >> 0x1f,uVar4,uVar17);
    lVar26 = st::fn_0072E4F0(uVar18,(int)uVar18 >> 0x1f,uVar4,uVar17);
    lVar24 = st::fn_0072E4F0(uVar16,iVar10,uVar5,uVar19);
    lVar24 = st::fn_0072E4F0
                       ((uint)(lVar24 - lVar26),(int)((ulonglong)(lVar24 - lVar26) >> 0x20),uVar5,
                        uVar19);
    lVar27 = st::fn_0072E4F0(uVar11,uVar15,uVar4,uVar17);
    lVar22 = st::fn_0072E4F0(uVar16,iVar10,uVar6,uVar21);
    lVar22 = st::fn_0072E4F0
                       ((uint)(lVar22 - lVar23),(int)((ulonglong)(lVar22 - lVar23) >> 0x20),uVar6,
                        uVar21);
    lVar22 = (lVar27 - lVar24) - lVar22;
    lVar27 = st::fn_0072E4F0(uVar6,uVar21,uVar6,uVar21);
    lVar28 = st::fn_0072E4F0(uVar5,uVar19,uVar5,uVar19);
    lVar24 = st::fn_0072E4F0(uVar4,uVar17,uVar4,uVar17);
    lVar24 = lVar27 + lVar28 + lVar24;
    uVar25 = st::fn_0072E440
                       ((uint)lVar22,(uint)((ulonglong)lVar22 >> 0x20),(uint)lVar24,
                        (uint)((ulonglong)lVar24 >> 0x20));
    uVar20 = -(int)uVar25;
    iVar14 = -((int)((ulonglong)uVar25 >> 0x20) + (uint)((int)uVar25 != 0));
    iVar10 = (iVar14 - iVar10) - (uint)(uVar20 < uVar16);
    lVar24 = st::fn_0072E4F0(uVar20 - uVar16,iVar10,uVar5,uVar19);
    local_48 = st::fn_0072E440
                         ((uint)(lVar24 + lVar26),(uint)((ulonglong)(lVar24 + lVar26) >> 0x20),uVar4
                          ,uVar17);
    lVar24 = st::fn_0072E4F0(uVar20 - uVar16,iVar10,uVar6,uVar21);
    uVar25 = st::fn_0072E440
                       ((uint)(lVar24 + lVar23),(uint)((ulonglong)(lVar24 + lVar23) >> 0x20),uVar4,
                        uVar17);
  }
  uVar4 = uVar7 - (uint)uVar25;
  iVar12 = (((int)uVar7 >> 0x1f) - (int)((ulonglong)uVar25 >> 0x20)) - (uint)(uVar7 < (uint)uVar25);
  iVar13 = (((int)uVar8 >> 0x1f) - STPiece<4,4>(local_48)) - (uint)(uVar8 < (uint)local_48);
  iVar14 = (((int)uVar9 >> 0x1f) - iVar14) - (uint)(uVar9 < uVar20);
  iVar10 = ((int)*(short *)&this->field_0x237 << 2) / 5;
  lVar24 = st::fn_0072E4F0(uVar9 - uVar20,iVar14,uVar9 - uVar20,iVar14);
  lVar22 = st::fn_0072E4F0(uVar8 - (uint)local_48,iVar13,uVar8 - (uint)local_48,iVar13);
  lVar23 = st::fn_0072E4F0(uVar4,iVar12,uVar4,iVar12);
  if ((lVar22 + lVar24 + lVar23 <= (longlong)(iVar10 * iVar10)) &&
     (lVar22 = (longlong)(int)(uVar8 - (int)param_1->field_0043) *
               (longlong)((int)sVar2 - (int)param_1->field_0043) +
               (longlong)(int)(uVar9 - (int)param_1->field_0041) *
               (longlong)((int)sVar1 - (int)param_1->field_0041),
     lVar24 = (longlong)(int)(uVar7 - (int)param_1->field_0045) *
              (longlong)((int)sVar3 - (int)param_1->field_0045),
     -1 < (int)((int)((ulonglong)lVar22 >> 0x20) + (int)((ulonglong)lVar24 >> 0x20) +
               (uint)CARRY4((uint)lVar22,(uint)lVar24)))) {
    return 1;
  }
  return 0;
}

